-- WarehouseOS 2.1.0 - SpawnNet 2.2.1 Core-Hosted Edition
local net=dofile('/spawnnet/client/net.lua')
local auth=dofile('/spawnnet/client/auth_client.lua')
local C=colors
local DOMAIN='warehouse'
if not fs.exists('/spawnnet/client/package_manager.lua')then error('WarehouseOS 2.1.0 requires SpawnNet 2.2.1+ with native application packages.',0)end
...
do
 local p=base('Warehouse Terminal','Search, withdraw, deposit, and rename item variants','terminal');nav(p);p.liveInterval=2;p.liveServerEvent='terminal_refresh'
 ...
 add(p,{type='input',id='searchText',x=3,y=12,w=23,value='',placeholder='Search name / ID'})
 ...
 add(p,{type='button',x=3,y=16,w=12,text='< PAGE',bg=C.gray,action={type='event',event='search_prev'}})
 add(p,{type='text',id='searchState',x=16,y=16,w=21,text='Search results below',align='center'})
 add(p,{type='button',x=38,y=16,w=11,text='PAGE >',bg=C.gray,action={type='event',event='search_next'}})
 for i=1,8 do add(p,{type='button',id='item'..i,x=3,y=17+i*2,w=45,text='-',bg=i%2==0 and C.gray or C.black,visible=false,action={type='event',event='pick_item'..i}})end
 add(p,{type='text',id='selectedItem',x=3,y=35,w=45,text='Selected: none'})
 add(p,{type='text',id='selectedDetails',x=3,y=37,w=45,h=2,text='Registry / metadata details appear here.',fg=C.lightGray})
 add(p,{type='input',id='aliasName',x=3,y=40,w=24,value='',placeholder='Custom display name'})
 add(p,{type='button',x=28,y=40,w=10,text='SAVE',bg=C.purple,action={type='server',event='alias_save'}})
 add(p,{type='button',x=39,y=40,w=10,text='RESET',bg=C.gray,action={type='server',event='alias_reset'}})
 add(p,{type='input',id='quantity',x=3,y=43,w=12,value='64'})
 add(p,{type='button',x=16,y=43,w=16,text='WITHDRAW',bg=C.lime,fg=C.black,action={type='server',event='withdraw_submit'}})
 add(p,{type='button',x=33,y=43,w=16,text='DEPOSIT',bg=C.cyan,fg=C.black,action={type='server',event='deposit_submit'}})
 add(p,{type='text',id='targetState',x=3,y=46,w=45,text='Delivery target: Main Warehouse'})
 add(p,{type='button',x=3,y=48,w=14,text='MAIN',bg=C.blue,action={type='event',event='target_main'}})
 add(p,{type='button',id='target1',x=18,y=48,w=15,text='-',visible=false,bg=C.gray,action={type='event',event='target_1'}})
 add(p,{type='button',id='target2',x=34,y=48,w=15,text='-',visible=false,bg=C.gray,action={type='event',event='target_2'}})
 add(p,{type='button',id='target3',x=18,y=50,w=15,text='-',visible=false,bg=C.gray,action={type='event',event='target_3'}})
 add(p,{type='button',id='target4',x=34,y=50,w=15,text='-',visible=false,bg=C.gray,action={type='event',event='target_4'}})
 add(p,{type='text',id='operationStatus',x=3,y=53,w=45,h=2,text='Ready.'})
 add(p,{type='button',x=3,y=57,w=14,text='MEMBERS',bg=C.purple,action={type='navigate',target='spn://warehouse/members'}})
 add(p,{type='button',x=18,y=57,w=14,text='HISTORY',bg=C.gray,action={type='navigate',target='spn://warehouse/history'}})
 add(p,{type='button',x=33,y=57,w=16,text='SETTINGS',bg=C.gray,action={type='navigate',target='spn://warehouse/settings'}})
 pages['/terminal']=p
end
do
 local p=base('Warehouse Settings','Controller pairing, remote terminals, and deletion','settings');nav(p);hidden(p,'warehouseId','')
 ...
 add(p,{type='button',x=3,y=30,w=45,text='INSTALL / UPDATE WAREHOUSEOS HERE',bg=C.lime,fg=C.black,action={type='install',package='warehouseos'}})
 add(p,{type='heading',x=3,y=34,w=45,text='REMOTE TERMINALS',fg=C.yellow})
 add(p,{type='text',id='terminal1',x=3,y=36,w=45,text='-'})
 add(p,{type='text',id='terminal2',x=3,y=38,w=45,text='-'})
 add(p,{type='text',id='terminal3',x=3,y=40,w=45,text='-'})
 add(p,{type='text',id='terminal4',x=3,y=42,w=45,text='-'})
 add(p,{type='heading',x=3,y=46,w=45,text='DANGER ZONE',fg=C.red})
 add(p,{type='input',id='deleteConfirm',x=3,y=49,w=45,value='',placeholder='Type DELETE'})
 add(p,{type='button',x=3,y=52,w=45,text='DELETE WAREHOUSE',bg=C.red,action={type='server',event='delete_warehouse_submit'}})
 add(p,{type='text',id='deleteStatus',x=3,y=55,w=45,h=3,text='Deletes the WarehouseOS record. Physical chest contents are NEVER deleted.'})
 pages['/settings']=p
end
do
...
do
 ...
 for i=1,8 do add(p,{type='text',id='history'..i,x=3,y=9+i*3,w=45,h=2,text='-'})end
 add(p,{type='button',x=3,y=36,w=45,text='REFRESH HISTORY',bg=C.gray,action={type='server',event='history_refresh'}})
 pages['/history']=p
end
do
...
call db.get "history-${input.warehouseId}" "h4" -> o4
call db.get "history-${input.warehouseId}" "h5" -> o5
call db.get "history-${input.warehouseId}" "h6" -> o6
call db.get "history-${input.warehouseId}" "h7" -> o7
call db.set "history-${input.warehouseId}" "h8" $o7
call db.set "history-${input.warehouseId}" "h7" $o6
call db.set "history-${input.warehouseId}" "h6" $o5
call db.set "history-${input.warehouseId}" "h5" $o4
...
call ui.setText "warehouseState" "ONLINE - ${c.inventories} storage inventories / ${c.totalItems} items"
endif
endif
...
call db.get "commands" $input.warehouseId -> pending
if $pending != nil
call ui.alert "Controller is busy."
else
...
set cmd.user $who
call db.set "commands" $input.warehouseId $cmd
call ui.setText "operationStatus" "Main deposit queued."
endif
else
call db.get "terms-${input.warehouseId}" $input.terminal -> t
if $t == nil
...
call db.get "terminalcommands" $ck -> pending
if $pending != nil
call ui.alert "Remote terminal is busy."
else
call db.get "controllers" $input.warehouseId -> c
if $c == nil
...
call ui.setText "operationStatus" "Remote deposit queued."
endif
endif
endif
endif
...
call db.get "roles-${input.warehouseId}" $who -> role
if $role == nil
call ui.setText "memberStatus" "NO ACCESS"
else
call ui.setText "memberStatus" "Use Grant / Update Access to share this warehouse."
endif
end
event history_refresh
call ui.setText "history1" "-"
call ui.setText "history2" "-"
call ui.setText "history3" "-"
call ui.setText "history4" "-"
call ui.setText "history5" "-"
call ui.setText "history6" "-"
call ui.setText "history7" "-"
call ui.setText "history8" "-"
call user.name -> who
call db.get "roles-${input.warehouseId}" $who -> role
if $role == nil
...
if $h.h5 != nil
call ui.setText "history5" "${h.h5.action}: ${h.h5.message}"
endif
if $h.h6 != nil
call ui.setText "history6" "${h.h6.action}: ${h.h6.message}"
endif
if $h.h7 != nil
call ui.setText "history7" "${h.h7.action}: ${h.h7.message}"
endif
if $h.h8 != nil
call ui.setText "history8" "${h.h8.action}: ${h.h8.message}"
endif
...
local mode=(...)or'setup';local CFG='controller.db';local cfg=common.load(CFG,nil)
...
local function configure(existing)
 ...
 cfg={warehouseId=p.result.warehouseId,warehouseName=p.result.warehouseName,token=token,name=name,output=output,deposit=deposit,autoDeposit=true};common.save(CFG,cfg);gui.toast('Paired to '..tostring(cfg.warehouseName)..'.',3,true);return true
end
...
if mode=='reset'then common.save(CFG,nil);gui.toast('Controller pairing removed from this computer.',2,true);return true end
...
local q={text='',mod='',min=0,sort='name',page=1,user=''};local aliases={}
local function makeView()
 local text=tostring(q.text or''):lower();local mod=tostring(q.mod or''):lower();local min=tonumber(q.min)or 0;local a={}
 for _,it in pairs(index)do local alias=aliases[it.key]or'';local name=alias~=''and alias or it.originalName;local hay=(name..' '..it.originalName..' '..it.id..' '..it.mod..' '..it.meta):lower();if(text==''or hay:find(text,1,true))and(mod==''or tostring(it.mod):lower():find(mod,1,true))and it.amount>=min then a[#a+1]={key=it.key,name=name,alias=alias,originalName=it.originalName,id=it.id,meta=it.meta,mod=it.mod,amount=it.amount,stacks=it.stacks,details=it.id..'  meta '..it.meta..'  ['..it.mod..']  original: '..it.originalName}end end
 if q.sort=='amount'then table.sort(a,function(x,y)if x.amount==y.amount then return x.name<y.name end;return x.amount>y.amount end)else table.sort(a,function(x,y)return tostring(x.name)<tostring(y.name)end)end
...
local lastCmd,lastAuto=nil,0;scan()
while true do
 local fresh=common.load(CFG,nil);if not fresh then return end;cfg=fresh
 local stores=scan();local total=0;for _,it in pairs(index)do total=total+it.amount end
 local view=makeView();local p=common.web('controller_heartbeat',{warehouseId=cfg.warehouseId,token=cfg.token,computerId=os.getComputerID(),name=cfg.name,output=cfg.output,deposit=cfg.deposit,outputOnline=cfg.output and peripheral.isPresent(cfg.output)or false,depositOnline=cfg.deposit and peripheral.isPresent(cfg.deposit)or false,firstStorage=stores[1],inventories=#stores,totalItems=total,view=view})
 local r=p and p.result
 if type(r)=='table'then
  if r.deleted or r.replaced then common.save(CFG,nil);return end
  if r.ok then if type(r.query)=='table'then q=r.query end;aliases=aliasMap(r.aliases);local cmd=r.command;if type(cmd)=='table'and cmd.requestId and cmd.requestId~=lastCmd then lastCmd=cmd.requestId;local ok,moved,msg;if cmd.action=='withdraw'then ok,moved,msg=withdraw(cmd.item,cmd.amount,cmd.targetName or cfg.output)elseif cmd.action=='deposit'then ok,moved,msg=deposit(cfg.deposit)else ok,moved,msg=false,0,'Unsupported controller action'end;common.web('controller_result',{warehouseId=cfg.warehouseId,token=cfg.token,requestId=cmd.requestId,action=cmd.action,ok=ok,moved=moved or 0,message=msg or'Complete',item=cmd.item,user=cmd.user})end end
 end
 local now=os.clock();if cfg.autoDeposit and cfg.deposit and now-lastAuto>3 then lastAuto=now;deposit(cfg.deposit)end
 sleep(1.5)
end]==],
  ['terminal.lua']=[==[local gui=dofile('/spawnnet/client/gui.lua')
local crypto=dofile('/spawnnet/lib/crypto.lua')
local common=dofile('/spawnnet/apps/warehouse/warehouseos/app/common.lua')
local mode=(...)or'setup';local CFG='terminal.db';local cfg=common.load(CFG,nil)
...
local function configure(existing)
 ...
 local code=nil
 if not existing then code=gui.prompt('PAIR REMOTE TERMINAL','Pair code from Warehouse Settings:','');if code==''then return false end end
 ...
 if not p or type(p.result)~='table'or not p.result.ok then gui.toast((p and p.result and p.result.error)or e or'Pair failed',4,false);return false end
 cfg={warehouseId=p.result.warehouseId,warehouseName=p.result.warehouseName,terminalId=p.result.terminalId,slot=p.result.terminalSlot,token=token,name=name,inventory=inv};common.save(CFG,cfg);gui.toast('Paired '..cfg.name..' as '..cfg.terminalId..'.',3,true);return true
end
if mode=='setup'then if cfg and cfg.token then return configure(cfg)else return configure(nil)end end
if mode=='configure'then if not cfg then gui.toast('Remote terminal is not paired yet.',2,false);return false end;return configure(cfg)end
if mode=='reset'then common.save(CFG,nil);gui.toast('Remote terminal pairing removed from this computer.',2,true);return true end
if mode~='service'then return end
if not cfg or not cfg.token then return end
local function push(src,target,slot,amount)local ok,n=pcall(peripheral.call,src,'pushItems',target,slot,amount);if ok and tonumber(n)then return tonumber(n)end;return 0 end
...
local function deposit(target)
 if not peripheral.isPresent(cfg.inventory)then return false,0,'Remote terminal inventory is missing'end
 local ok,l=pcall(peripheral.call,cfg.inventory,'list');if not ok then return false,0,'Remote terminal inventory offline'end;local moved=0
 for slot,it in pairs(l or{})do moved=moved+push(cfg.inventory,target,tonumber(slot),tonumber(it.count)or 64)end
 if moved<=0 then return false,0,'No physical item path to warehouse storage.'end;return true,moved,'Deposited '..moved..' items from '..cfg.name
end
local lastCmd=nil
while true do
 local fresh=common.load(CFG,nil);if not fresh then return end;cfg=fresh
 local p=common.web('terminal_heartbeat',{warehouseId=cfg.warehouseId,terminalId=cfg.terminalId,slot=cfg.slot,token=cfg.token,computerId=os.getComputerID(),name=cfg.name,inventory=cfg.inventory,count=count()});local r=p and p.result
 if type(r)=='table'then
  if r.deleted or r.removed or r.replaced then common.save(CFG,nil);return end
  if r.ok and type(r.command)=='table'and r.command.requestId and r.command.requestId~=lastCmd then local cmd=r.command;lastCmd=cmd.requestId;local ok,moved,msg;if cmd.action=='deposit'then ok,moved,msg=deposit(cmd.targetName)else ok,moved,msg=false,0,'Unsupported remote command'end;common.web('terminal_result',{warehouseId=cfg.warehouseId,terminalId=cfg.terminalId,token=cfg.token,requestId=cmd.requestId,action=cmd.action,ok=ok,moved=moved or 0,message=msg or'Complete',user=cmd.user})end
 end
 sleep(1.5)
end]==],
...
local function controller()
 while true do
  if common.load('controller.db',nil)then os.run({},APP..'controller.lua','service')else sleep(2)end
  sleep(1)
 end
end
local function terminal()
 while true do
  if common.load('terminal.db',nil)then os.run({},APP..'terminal.lua','service')else sleep(2)end
  sleep(1)
 end
end
parallel.waitForAll(controller,terminal)]==],
...
local function diagnostics()
 ...
 lines[#lines+1]='WarehouseOS 2.1.0 diagnostics';lines[#lines+1]='Wireless modem: '..tostring(common.findWirelessModem()or'NOT FOUND');lines[#lines+1]='Inventory peripherals: '..#inv
 ...
 if c then lines[#lines+1]='  Output: '..tostring(c.output);lines[#lines+1]='  Deposit: '..tostring(c.deposit or'(none)')end
...
while true do
 ...
 local m=gui.menu('WAREHOUSEOS 2.1','SpawnNet manages the background service. No dedicated Host computer.',items)
...
print('WarehouseOS 2.1.0 preflight: OK')
...
call('web','publish',{domain=DOMAIN,note='WarehouseOS 2.1.0 - SpawnNet 2.2.1 compatibility and reliability rebuild'})
print('Publishing installable WarehouseOS package...')
call('package','publish',{
 domain=DOMAIN,name='warehouseos',title='WarehouseOS',version='2.1.0',
...
term.setTextColor(colors.lime);print('WAREHOUSEOS 2.1.0 PUBLISHED');term.setTextColor(colors.white)
...
print('Background service: SpawnNet-managed; reboot once after first setup.')

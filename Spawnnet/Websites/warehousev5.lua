-- WarehouseOS 2.3.0 - SpawnNet 2.2.2 Core-Hosted Edition
local net=dofile('/spawnnet/client/net.lua')
local auth=dofile('/spawnnet/client/auth_client.lua')
local C=colors
local DOMAIN='warehouse'
if not fs.exists('/spawnnet/client/package_manager.lua')then error('WarehouseOS 2.3.0 requires SpawnNet 2.2.2+ with native application packages.',0)end
...
local function diagnostics()
 ...
 lines[#lines+1]='WarehouseOS 2.3.0 diagnostics';lines[#lines+1]='Wireless modem: '..tostring(common.findWirelessModem()or'NOT FOUND');lines[#lines+1]='Inventory peripherals: '..#inv
...
while true do
 ...
 local m=gui.menu('WAREHOUSEOS 2.3','SpawnNet manages the background service. No dedicated Host computer.',items)
...
print('WarehouseOS 2.3.0 preflight: OK')
...
call('web','publish',{domain=DOMAIN,note='WarehouseOS 2.3.0 - heartbeat transaction protocol, quick withdraw, filters, auto-deposit control'})
print('Publishing installable WarehouseOS package...')
call('package','publish',{
 domain=DOMAIN,name='warehouseos',title='WarehouseOS',version='2.3.0',
...
term.setTextColor(colors.lime);print('WAREHOUSEOS 2.3.0 PUBLISHED');term.setTextColor(colors.white)

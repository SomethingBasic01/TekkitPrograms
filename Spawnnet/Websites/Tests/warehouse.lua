...
do
 local p=base('Warehouse Terminal','Live inventory browser and main-controller operations','terminal');nav(p);p.liveInterval=0.5
 ...
 add(p,{type='badge',id='ioState',x=3,y=21,w=45,text='I/O: MAIN OUTPUT + MAIN DEPOSIT',bg=C.blue,align='center'})
...
if $input.result.requestId != nil
math receiptSlot $input.result.requestId % 64
concat receiptKey "c" $receiptSlot
call db.get "receipts-${input.warehouseId}" $receiptKey -> seen
if $seen != $input.result.requestId
set res.requestId $input.result.requestId
set res.ok $input.result.ok
set res.moved $input.result.moved
set res.message $input.result.message
set res.action $input.result.action
set res.item $input.result.item
set res.user $input.result.user
set res.time $now
...
call db.set "receipts-${input.warehouseId}" $receiptKey $input.result.requestId
endif
set out.resultAck $input.result.requestId
endif
...
call ui.alert "Controller is collecting the previous request. Try again in a second."
else
...
call ui.setText "operationStatus" "Withdrawal queued for Main Output."
endif
endif
endif
endif
...
set cmd.user $who
call db.set "commands" $input.warehouseId $cmd
call ui.setText "operationStatus" "Main Deposit sweep queued."
endif
endif
endif
endif
...
call db.get "terminallast-${input.warehouseId}" $terms.slot1 -> tl
math ta $now - $tl
if $ta > 10
call ui.setText "terminal1" "${terms.slot1} - ${tn} [OFFLINE]"
else
call ui.setText "terminal1" "${terms.slot1} - ${tn} [ONLINE]"
endif
endif
call ui.setText "terminal2" "-"
if $terms.slot2 != nil
call db.get "terminalnames-${input.warehouseId}" $terms.slot2 -> tn
call db.get "terminallast-${input.warehouseId}" $terms.slot2 -> tl
math ta $now - $tl
if $ta > 10
call ui.setText "terminal2" "${terms.slot2} - ${tn} [OFFLINE]"
else
call ui.setText "terminal2" "${terms.slot2} - ${tn} [ONLINE]"
endif
endif
call ui.setText "terminal3" "-"
if $terms.slot3 != nil
call db.get "terminalnames-${input.warehouseId}" $terms.slot3 -> tn
call db.get "terminallast-${input.warehouseId}" $terms.slot3 -> tl
math ta $now - $tl
if $ta > 10
call ui.setText "terminal3" "${terms.slot3} - ${tn} [OFFLINE]"
else
call ui.setText "terminal3" "${terms.slot3} - ${tn} [ONLINE]"
endif
endif
call ui.setText "terminal4" "-"
if $terms.slot4 != nil
call db.get "terminalnames-${input.warehouseId}" $terms.slot4 -> tn
call db.get "terminallast-${input.warehouseId}" $terms.slot4 -> tl
math ta $now - $tl
if $ta > 10
call ui.setText "terminal4" "${terms.slot4} - ${tn} [OFFLINE]"
else
call ui.setText "terminal4" "${terms.slot4} - ${tn} [ONLINE]"
endif
endif
endif
end
...
function M.selectWarehouse(id,name)local util=dofile('/spawnnet/lib/util.lua');M.ensure('/spawnnet/local');local p='/spawnnet/local/warehouse.db';local st=util.loadTable(p,{});util.tablePathSet(st,'warehouse.selected',id);util.tablePathSet(st,'warehouse.name',name);util.saveTable(p,st)end
...
local function configure(existing)
 ...
 if existing then existing.output=output;existing.deposit=deposit;existing.name=name;existing.autoDeposit=existing.autoDeposit~=false;common.save(CFG,existing);common.selectWarehouse(existing.warehouseId,existing.warehouseName);gui.toast('Controller I/O configuration saved.',2,true);return true end
 ...
 cfg={warehouseId=p.result.warehouseId,warehouseName=p.result.warehouseName,token=token,name=name,output=output,deposit=deposit,autoDeposit=true};common.save(CFG,cfg);common.selectWarehouse(cfg.warehouseId,cfg.warehouseName);common.save(RUN,nil);common.save('controller.result',nil);gui.toast('Paired to '..tostring(cfg.warehouseName)..'.',3,true);return true
end
...
local lastAuto=0;local lastRuntime=0;scan()
while true do
 local fast=false
 ...
 if type(r)=='table'then
  ...
  if r.ok then
   if pendingResult and r.resultAck==pendingResult.requestId then common.save(RESULT,nil);pendingResult=nil end
   ...
   local cmd=r.command;if not pendingResult and type(cmd)=='table'and cmd.requestId then local ok,moved,msg;if cmd.action=='withdraw'then ok,moved,msg=withdraw(cmd.item,cmd.amount,cfg.output)elseif cmd.action=='deposit'then ok,moved,msg=deposit(cfg.deposit)else ok,moved,msg=false,0,'Unsupported controller action'end;local res={requestId=cmd.requestId,action=cmd.action,ok=ok,moved=moved or 0,message=msg or'Complete',item=cmd.item,user=cmd.user};common.save(RESULT,res);fast=true end
  end
 end
 local now=os.clock();if cfg.autoDeposit~=false and cfg.deposit and now-lastAuto>3 then lastAuto=now;local a,m=deposit(cfg.deposit);if a and (tonumber(m)or 0)>0 then fast=true end end
...
local function configure(existing)
 ...
 local token=crypto.randomHex(24);local p,e=common.web('terminal_pair',{code=code,token=token,computerId=os.getComputerID(),name=name})
 if not p or type(p.result)~='table'or not p.result.ok then gui.toast((p and p.result and p.result.error)or e or'Pair failed',4,false);return false end
 cfg={warehouseId=p.result.warehouseId,warehouseName=p.result.warehouseName,terminalId=p.result.terminalId,slot=p.result.terminalSlot,token=token,name=name};common.save(CFG,cfg);common.selectWarehouse(cfg.warehouseId,cfg.warehouseName);common.save(RUN,nil);common.save('terminal.result',nil);gui.toast('Paired '..cfg.name..' as a remote access console.',3,true);return true
end
if mode=='setup'then if cfg and cfg.token then return configure(cfg)else return configure(nil)end end
...
common.save(RUN,{running=true,started=common.epoch(),warehouseId=cfg.warehouseId})
while true do
 local fresh=common.load(CFG,nil);if not fresh then common.save(RUN,nil);return end;cfg=fresh
 common.save(RUN,{running=true,lastSeen=common.epoch(),warehouseId=cfg.warehouseId})
 local p=common.web('terminal_heartbeat',{warehouseId=cfg.warehouseId,terminalId=cfg.terminalId,token=cfg.token,computerId=os.getComputerID(),name=cfg.name});local r=p and p.result
 if type(r)=='table'then
  if r.deleted or r.removed or r.replaced then common.save(CFG,nil);common.save(RUN,nil);return end
 end
 sleep(2)
end]==],
...
local function ensureAutoStart()
 local ok,sm=pcall(dofile,'/spawnnet/client/service_manager.lua');if not ok or type(sm)~='table'then return false end
 sm.register('warehouse','warehouseos','WarehouseOS',APP..'service.lua');return sm.runtimeActive()
end
ensureAutoStart()
local function diagnostics()
 local c=common.load('controller.db',nil);local t=common.load('terminal.db',nil);local cr=common.load('controller.runtime',nil);local tr=common.load('terminal.runtime',nil);local inv=common.inventoryNames();local lines={}
 ...
 lines[#lines+1]='Remote access terminal: '..(t and((t.warehouseName or t.warehouseId)..' / '..tostring(t.name)..' / '..tostring(t.terminalId))or'not configured')
 if t then lines[#lines+1]='  Worker heartbeat: '..(tr and tostring(math.max(0,common.epoch()-(tr.lastSeen or tr.started or 0)))..'s ago'or'not running');lines[#lines+1]='  Physical item transfer: DISABLED BY DESIGN'end
 local ok,sm=pcall(dofile,'/spawnnet/client/service_manager.lua');lines[#lines+1]='SpawnNet service runtime: '..(ok and sm.runtimeActive()and'ACTIVE'or'NOT RUNNING')
...
while true do
 ...
 items[#items+1]={label='Repair / enable automatic controller startup',action='autostart'}
 items[#items+1]={label='Reboot now',action='reboot'}
 items[#items+1]={label='Exit',action='exit'}
 local m=gui.menu('WAREHOUSEOS 2.4.0','Controller watchdog is registered for automatic startup and restart.',items)
 ...
 elseif m.action=='web'then local x=c or t;if x then common.selectWarehouse(x.warehouseId,x.warehouseName)end;os.run({},'/spawnnet/client/browser.lua',x and'spn://warehouse/terminal'or'spn://warehouse')
 ...
 elseif m.action=='autostart'then local active=ensureAutoStart();gui.toast(active and'Auto-start enabled; watchdog is active now.'or'Auto-start repaired. Reboot once to start the service runtime.',4,active)
 elseif m.action=='reboot'then if gui.confirm('REBOOT','WarehouseOS is registered to start automatically under SpawnNet. Reboot now?')then os.reboot()end end
end]==],
}
...
local repair240=dbGet('warehouseos-meta','2.4.0-serializer-repair')
if not repair240 then
 ...
 dbSet('warehouseos-meta','2.4.0-serializer-repair',true)
...
call('web','publish',{domain=DOMAIN,note='WarehouseOS 2.4.0 - serializer repair, true live refresh, access-only remote terminals, and controller watchdog'})
...
call('package','publish',{
 ...
 description='Physical warehouse controller with live web inventory, access-only remote consoles, and an automatically supervised controller service.',

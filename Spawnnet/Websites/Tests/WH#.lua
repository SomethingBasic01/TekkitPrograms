-- WarehouseOS 2.5.1 - SpawnNet 2.3.0 Citadel Edition
local net=dofile('/spawnnet/client/net.lua')
local auth=dofile('/spawnnet/client/auth_client.lua')
local C=colors
local DOMAIN='warehouse'
if not fs.exists('/spawnnet/client/package_manager.lua')then error('WarehouseOS 2.5.1 requires SpawnNet 2.3.0+ with native application packages.',0)end
...
local function base(title,sub,mode)
 ...
 add(p,{type='panel',x=1,y=1,w='100%',h=4,bg=C.purple,children={
  {type='heading',x=2,y=1,w=47,text='WH// WAREHOUSEOS 2.5.1',fg=C.white,bg=C.purple},
...
local function configure(existing)
 ...
 cfg={warehouseId=p.result.warehouseId,warehouseName=p.result.warehouseName,token=token,name=name,output=output,deposit=deposit,autoDeposit=true};common.save(CFG,cfg);common.selectWarehouse(cfg.warehouseId,cfg.warehouseName);common.save(RUN,nil);common.save('controller.result',nil);gui.toast('CIPHER ACCEPTED // '..tostring(cfg.warehouseName)..' ONLINE',3,true);return true
end
...
local lastAuto=0;local lastRuntime=0;local lastView=0;scan()
while true do
 local fast=false
 ...
 if type(r)=='table'then
  ...
  if r.ok then
   ...
   local vnow=os.clock();if vnow-lastView>=0.75 then lastView=vnow;local view=makeView();common.web('controller_view',{warehouseId=cfg.warehouseId,token=cfg.token,view=view})end
  end
...
local function configure(existing)
 ...
 if not p or type(p.result)~='table'or not p.result.ok then gui.toast((p and p.result and p.result.error)or e or'Pair failed',4,false);return false end
 cfg={warehouseId=p.result.warehouseId,warehouseName=p.result.warehouseName,terminalId=p.result.terminalId,slot=p.result.terminalSlot,token=token,name=name};common.save(CFG,cfg);common.selectWarehouse(cfg.warehouseId,cfg.warehouseName);common.save(RUN,nil);common.save('terminal.result',nil);gui.toast('REMOTE COMMAND LINK ESTABLISHED // '..cfg.name,3,true);return true
end
if mode=='setup'then if cfg and cfg.token then return configure(cfg)else return configure(nil)end end
...
local function diagnostics()
 ...
 lines[#lines+1]='WarehouseOS 2.5.1 diagnostics';lines[#lines+1]='Wireless modem: '..tostring(common.findWirelessModem()or'NOT FOUND');lines[#lines+1]='Inventory peripherals: '..#inv
...
while true do
 ...
 local m=gui.menu('WH// COMMAND CONSOLE 2.5.1','PHYSICAL STORAGE LINK // SUPERVISED // ENCRYPTED',items)
...
print('WarehouseOS 2.5.1 preflight: OK')
...
call('web','publish',{domain=DOMAIN,note='WarehouseOS 2.5.1 - persistent operation queue, isolated live inventory channel, independent auto-deposit, and controller watchdog'})
print('Publishing installable WarehouseOS package...')
call('package','publish',{
 domain=DOMAIN,name='warehouseos',title='WarehouseOS',version='2.5.1',
 description='Physical warehouse controller with reliable queued I/O, live web inventory, access-only remote consoles, and an automatically supervised controller service.',
...
term.setTextColor(colors.lime);print('WAREHOUSEOS 2.5.1 PUBLISHED');term.setTextColor(colors.white)

-- WarehouseOS 2.2.0 - SpawnNet 2.2.2 Core-Hosted Edition
local net=dofile('/spawnnet/client/net.lua')
local auth=dofile('/spawnnet/client/auth_client.lua')
local C=colors
local DOMAIN='warehouse'
if not fs.exists('/spawnnet/client/package_manager.lua')then error('WarehouseOS 2.2.0 requires SpawnNet 2.2.2+ with native application packages.',0)end
...
while true do
 ...
 if type(r)=='table'then
  ...
  if r.ok then local pending=common.load(RUN,nil);if pending then ackResult(pending)elseif type(r.command)=='table'and r.command.requestId then local cmd=r.command;local ok,moved,msg;if cmd.action=='deposit'then ok,moved,msg=deposit(cmd.targetName)else ok,moved,msg=false,0,'Unsupported remote command'end;pending={warehouseId=cfg.warehouseId,terminalId=cfg.terminalId,token=cfg.token,requestId=cmd.requestId,action=cmd.action,ok=ok,moved=moved or 0,message=msg or'Complete',user=cmd.user};common.save(RUN,pending);ackResult(pending)end end
 end
 sleep(1.5)
end]==],
...
local function diagnostics()
 ...
 lines[#lines+1]='WarehouseOS 2.2.0 diagnostics';lines[#lines+1]='Wireless modem: '..tostring(common.findWirelessModem()or'NOT FOUND');lines[#lines+1]='Inventory peripherals: '..#inv
...
print('WarehouseOS 2.2.0 preflight: OK')
...
call('web','publish',{domain=DOMAIN,note='WarehouseOS 2.2.0 - reliable operation queue, quick withdraw, filters, auto-deposit control'})
print('Publishing installable WarehouseOS package...')
call('package','publish',{
 domain=DOMAIN,name='warehouseos',title='WarehouseOS',version='2.2.0',
...
term.setTextColor(colors.lime);print('WAREHOUSEOS 2.2.0 PUBLISHED');term.setTextColor(colors.white)

-- WarehouseOS 2.2.1 - SpawnNet 2.2.2 Core-Hosted Edition
local net=dofile('/spawnnet/client/net.lua')
local auth=dofile('/spawnnet/client/auth_client.lua')
local C=colors
local DOMAIN='warehouse'
if not fs.exists('/spawnnet/client/package_manager.lua')then error('WarehouseOS 2.2.1 requires SpawnNet 2.2.2+ with native application packages.',0)end
...
set pending.state "done"
call db.set "commands" $input.warehouseId $pending
endif
call db.get "results" $input.warehouseId -> oldres
if $oldres.requestId == $input.requestId
set out.ok true
else
...
if $pending.requestId == $input.requestId
set pending.state "done"
call db.set "terminalcommands" $ck $pending
endif
call db.get "results" $input.warehouseId -> oldres
if $oldres.requestId == $input.requestId
set out.ok true
...
call ui.alert "Please wait - the previous request is being acknowledged."
else
...
call db.get "terminalcommands" $ck -> pending
if $pending.state == "queued"
call ui.alert "Please wait - the remote terminal is acknowledging a request."
else
call db.get "controllers" $input.warehouseId -> c
...
local function configure(existing)
 ...
 cfg={warehouseId=p.result.warehouseId,warehouseName=p.result.warehouseName,token=token,name=name,output=output,deposit=deposit,autoDeposit=true};common.save(CFG,cfg);common.save(RUN,nil);gui.toast('Paired to '..tostring(cfg.warehouseName)..'.',3,true);return true
end
...
if mode=='reset'then common.save(CFG,nil);common.save(RUN,nil);gui.toast('Controller pairing removed from this computer.',2,true);return true end
...
local function ackResult(res)local p=common.web('controller_result',res);if p and type(p.result)=='table'and p.result.ok then common.save(RUN,nil);return true end;return false end
local lastAuto=0;scan()
while true do
 ...
 if type(r)=='table'then
  if r.deleted or r.replaced then common.save(RUN,nil);common.save(CFG,nil);return end
  if r.ok then
   ...
   local pending=common.load(RUN,nil)
   if pending then local c=r.command;if type(c)=='table'and c.requestId==pending.requestId then ackResult(pending)else common.save(RUN,nil)end else local cmd=r.command;if type(cmd)=='table'and cmd.requestId then local ok,moved,msg;if cmd.action=='withdraw'then ok,moved,msg=withdraw(cmd.item,cmd.amount,cmd.targetName or cfg.output)elseif cmd.action=='deposit'then ok,moved,msg=deposit(cfg.deposit)else ok,moved,msg=false,0,'Unsupported controller action'end;pending={warehouseId=cfg.warehouseId,token=cfg.token,requestId=cmd.requestId,action=cmd.action,ok=ok,moved=moved or 0,message=msg or'Complete',item=cmd.item,user=cmd.user};common.save(RUN,pending);ackResult(pending)end end
  end
 end
 local now=os.clock();if cfg.autoDeposit~=false and cfg.deposit and now-lastAuto>3 then lastAuto=now;deposit(cfg.deposit)end
 sleep(1.5)
end]==],
...
local function configure(existing)
 ...
 if not p or type(p.result)~='table'or not p.result.ok then gui.toast((p and p.result and p.result.error)or e or'Pair failed',4,false);return false end
 cfg={warehouseId=p.result.warehouseId,warehouseName=p.result.warehouseName,terminalId=p.result.terminalId,slot=p.result.terminalSlot,token=token,name=name,inventory=inv};common.save(CFG,cfg);common.save(RUN,nil);gui.toast('Paired '..cfg.name..' as '..cfg.terminalId..'.',3,true);return true
end
if mode=='setup'then if cfg and cfg.token then return configure(cfg)else return configure(nil)end end
...
if mode=='reset'then common.save(CFG,nil);common.save(RUN,nil);gui.toast('Remote-terminal pairing removed from this computer.',2,true);return true end
if mode~='service'then return end
if not cfg or not cfg.token then return end
local function push(src,target,slot,amount)local ok,n=pcall(peripheral.call,src,'pushItems',target,slot,amount);if ok and tonumber(n)then return tonumber(n)end;return 0 end
...
local function ackResult(res)local p=common.web('terminal_result',res);if p and type(p.result)=='table'and p.result.ok then common.save(RUN,nil);return true end;return false end
while true do
 local fresh=common.load(CFG,nil);if not fresh then common.save(RUN,nil);return end;cfg=fresh
 ...
 if type(r)=='table'then
  if r.deleted or r.removed or r.replaced then common.save(RUN,nil);common.save(CFG,nil);return end
  if r.ok then local pending=common.load(RUN,nil);if pending then local c=r.command;if type(c)=='table'and c.requestId==pending.requestId then ackResult(pending)else common.save(RUN,nil)end elseif type(r.command)=='table'and r.command.requestId then local cmd=r.command;local ok,moved,msg;if cmd.action=='deposit'then ok,moved,msg=deposit(cmd.targetName)else ok,moved,msg=false,0,'Unsupported remote command'end;pending={warehouseId=cfg.warehouseId,terminalId=cfg.terminalId,token=cfg.token,requestId=cmd.requestId,action=cmd.action,ok=ok,moved=moved or 0,message=msg or'Complete',user=cmd.user};common.save(RUN,pending);ackResult(pending)end end
 end
 sleep(1.5)
end]==],
...
local function diagnostics()
 ...
 lines[#lines+1]='WarehouseOS 2.2.1 diagnostics';lines[#lines+1]='Wireless modem: '..tostring(common.findWirelessModem()or'NOT FOUND');lines[#lines+1]='Inventory peripherals: '..#inv
...
while true do
 ...
 local m=gui.menu('WAREHOUSEOS 2.2','SpawnNet manages the background service. No dedicated Host computer.',items)
...
print('WarehouseOS 2.2.1 preflight: OK')
...
call('web','publish',{domain=DOMAIN,note='WarehouseOS 2.2.1 - acknowledgement-safe queue, quick withdraw, filters, auto-deposit control'})
print('Publishing installable WarehouseOS package...')
call('package','publish',{
 domain=DOMAIN,name='warehouseos',title='WarehouseOS',version='2.2.1',
...
term.setTextColor(colors.lime);print('WAREHOUSEOS 2.2.1 PUBLISHED');term.setTextColor(colors.white)
...
print('Background service: SpawnNet-managed. Use spawnnet services to verify/start it immediately.')

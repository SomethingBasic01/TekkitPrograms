-- WarehouseOS 2.3.1 - SpawnNet 2.2.2 Core-Hosted Edition
local net=dofile('/spawnnet/client/net.lua')
local auth=dofile('/spawnnet/client/auth_client.lua')
local C=colors
local DOMAIN='warehouse'
if not fs.exists('/spawnnet/client/package_manager.lua')then error('WarehouseOS 2.3.1 requires SpawnNet 2.2.2+ with native application packages.',0)end
...
do
 local p=base('Warehouse Terminal','Filter inventory and withdraw directly from each result','terminal');nav(p);p.liveInterval=0.75;p.liveServerEvent='terminal_refresh'
...
set slotRec.id $tid
set slotRec.name $input.name
call db.set "tslots-${pr.warehouseId}" $slot $slotRec
call db.set "paircodes" $input.code nil
...
set slotRec.id $input.terminalId
set slotRec.name $input.name
call db.set "tslots-${input.warehouseId}" $t.slot $slotRec
if $input.result.requestId != nil
math receiptSlot $input.result.requestId % 64
concat receiptKey "t" $receiptSlot
call db.get "receipts-${input.warehouseId}" $receiptKey -> seen
if $seen != $input.result.requestId
set res.requestId $input.result.requestId
set res.ok $input.result.ok
...
if $pending.state == "queued"
call ui.alert "Remote terminal is collecting the previous request. Try again in a second."
else
call db.get "controllers" $input.warehouseId -> c
...
while true do
 local fast=false
 local fresh=common.load(CFG,nil);if not fresh then common.save(RESULT,nil);return end;cfg=fresh
 ...
 if type(r)=='table'then
  if r.deleted or r.replaced then common.save(RESULT,nil);common.save(CFG,nil);return end
  if r.ok then
...
while true do
 ...
 local fresh=common.load(CFG,nil);if not fresh then common.save(RESULT,nil);return end;cfg=fresh
 local pendingResult=common.load(RESULT,nil)
 local p=common.web('terminal_heartbeat',{warehouseId=cfg.warehouseId,terminalId=cfg.terminalId,slot=cfg.slot,token=cfg.token,computerId=os.getComputerID(),name=cfg.name,inventory=cfg.inventory,count=count(),result=pendingResult});local r=p and p.result
 if type(r)=='table'then
  if r.deleted or r.removed or r.replaced then common.save(RESULT,nil);common.save(CFG,nil);return end
  if r.ok then
   if pendingResult and r.resultAck==pendingResult.requestId then common.save(RESULT,nil);pendingResult=nil end
   local cmd=r.command;if not pendingResult and type(cmd)=='table'and cmd.requestId then local ok,moved,msg;if cmd.action=='deposit'then ok,moved,msg=deposit(cmd.targetName)else ok,moved,msg=false,0,'Unsupported remote command'end;local res={requestId=cmd.requestId,action=cmd.action,ok=ok,moved=moved or 0,message=msg or'Complete',user=cmd.user};common.save(RESULT,res);fast=true end
  end
 end
 sleep(fast and 0.05 or 0.75)
end]==],
...
local function diagnostics()
 ...
 lines[#lines+1]='WarehouseOS 2.3.1 diagnostics';lines[#lines+1]='Wireless modem: '..tostring(common.findWirelessModem()or'NOT FOUND');lines[#lines+1]='Inventory peripherals: '..#inv
...
while true do
 ...
 local m=gui.menu('WAREHOUSEOS 2.3.1','SpawnNet manages the background service. No dedicated Host computer.',items)
  ...
  if type(wh)=='table'and wid and not wh.deleted then
   ...
   if not newHas then
    ...
    for _,t in ipairs(oldTerms)do
     if type(t)=='table'and t.id and si<=4 then t.slot='slot'..si;dbSet('terms-'..wid,t.id,t);dbSet('tslots-'..wid,'slot'..si,{id=t.id,name=t.name});si=si+1 end
    end
   end
   for _,sr in ipairs(dbList('tslots-'..wid,20))do
    local sk=type(sr)=='table'and sr.key or nil
    local sv=type(sr)=='table'and sr.value or nil
    if sk and type(sv)=='table'and sv.id then dbSet('tslots-'..wid,sk,{id=sv.id,name=sv.name})end
   end
...
print('WarehouseOS 2.3.1 preflight: OK')
...
call('web','publish',{domain=DOMAIN,note='WarehouseOS 2.3.1 - terminal serialization fix and responsive live inventory'})
print('Publishing installable WarehouseOS package...')
call('package','publish',{
 domain=DOMAIN,name='warehouseos',title='WarehouseOS',version='2.3.1',
...
term.setTextColor(colors.lime);print('WAREHOUSEOS 2.3.1 PUBLISHED');term.setTextColor(colors.white)

-- SpawnNet 2.3.0 COMPLETE CLIENT
...
term.setBackgroundColor(colors.black);term.clear();term.setCursorPos(1,1);term.setBackgroundColor(colors.purple);term.setTextColor(colors.white);term.clearLine();print(' SN// NEXUS CLIENT INSTALLER 2.3.0 ');term.setBackgroundColor(colors.black);term.setTextColor(colors.white)
local upgrading=fs.exists('/spawnnet/client/net.lua');print(upgrading and'IN-PLACE UPGRADE // APPS + SESSIONS PRESERVED'or'FRESH CLIENT // JOINING THE GRID');print('Validated '..validate(files,'Client')..' embedded Lua programs.');local backup=install(files,'client')
local util=dofile('/spawnnet/lib/util.lua');local config=dofile('/spawnnet/lib/config.lua');local net=dofile('/spawnnet/client/net.lua');local cfg=util.loadTable(config.clientConfig,{transport='rednet',timeout=config.requestTimeout,boot='desktop'});cfg.boot=cfg.boot or'desktop'
if not upgrading then write('Transport rednet/websocket [rednet]: ');local t=read();if t~=''then cfg.transport=t end;if cfg.transport~='rednet'and cfg.transport~='websocket'then error('Transport must be rednet or websocket',0)end;if cfg.transport=='websocket'then write('WebSocket URL: ');cfg.wsUrl=read()end;util.saveTable(config.clientConfig,cfg);if cfg.transport=='rednet'then print('Scanning for SpawnNet realms...');local found=net.discoverNetworks(1.5);if found then for _,n in ipairs(found)do print('  '..n.id..' // '..n.name..' // '..n.visibility)end;if #found==1 then net.addNetwork(found[1]);net.setActiveNetwork(found[1].id)end end end
else util.saveTable(config.clientConfig,cfg);print('Preserved networks, sessions, installed apps, appdata and local site data.')end
ensure('/spawnnet/version');writeFile('/spawnnet/version/client-release.txt',VERSION..'\n');dofile('/spawnnet/client/startup_config.lua').apply(cfg.boot)
...
print();term.setTextColor(colors.lime);print('NEXUS CLIENT READY // '..VERSION);term.setTextColor(colors.lightGray);print('SpawnNet Desktop now launches automatically at startup.');print('Change boot mode in: spawnnet settings');print('Backup: '..backup);print('Launch now: spawnnet')

-- SpawnNet 2.2.2 COMPLETE CLIENT
...
while true do
  local reg=sm.list();local items={}
  items[#items+1]={label='Runtime: '..(sm.runtimeActive()and'ACTIVE'or'NOT RUNNING'),disabled=true}
  local names={};for k,s in pairs(reg)do names[#names+1]={k=k,s=s}end;table.sort(names,function(a,b)return a.k<b.k end)
  for _,x in ipairs(names)do items[#items+1]={label=(x.s.enabled and'[ON] 'or'[OFF] ')..tostring(x.s.title or x.k),disabled=true}end
  if not sm.runtimeActive()then items[#items+1]={label='START BACKGROUND SERVICES NOW',action='start'}end
  items[#items+1]={label='View service error log',action='log'}
  items[#items+1]={label='Back',action='back'}
  local m=gui.menu('SPAWNNET SERVICES',tostring(#names)..' registered application service(s)',items)
  if not m or m.action=='back'then return
  elseif m.action=='start'then
    if gui.confirm('START SERVICES','SpawnNet will keep application services running underneath a normal CraftOS shell. Continue?')then shell.run('/spawnnet/client/startup_bridge.lua');return end
  elseif m.action=='log'then local s=readFile('/spawnnet/service-errors.log');gui.viewer('SERVICE ERRORS',s~=''and s or'No recorded service errors.','SpawnNet 2.2.2')end
end
]=],
...
-- Repair the 2.2.1 service bridge in-place. It launched a root shell with
-- os.run({}), causing CraftOS startup recursion. Preserve real user startup.
local function repairServiceBridge()
  local target='/startup.lua';if not fs.exists(target)then return end
  local h=fs.open(target,'r');local current=h and h.readAll()or'';if h then h.close()end
  if not current:find('SPAWNNET_SERVICE_BRIDGE_',1,true)then return end
  writeFile(target,"-- SPAWNNET_SERVICE_BRIDGE_2.2.2\nif not shell or not shell.run then error('SpawnNet service bridge requires CraftOS shell',0) end\nshell.run('/spawnnet/client/startup_bridge.lua')\n")
end
repairServiceBridge()

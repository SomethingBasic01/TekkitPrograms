-- SpawnNet Storage Node 2.2.2 Complete Installer / Upgrader
local FILES={
...
local function writef(p,d)ensure(fs.getDir(p));local t=p..'.install-tmp';if fs.exists(t)then fs.delete(t)end;local h=assert(fs.open(t,'w'));h.write(d);h.close();if fs.exists(p)then fs.delete(p)end;fs.move(t,p)end
for p,d in pairs(FILES)do local f,e=loadstring(d,'@'..p);if not f then error('Embedded '..p..' failed CraftOS syntax validation: '..tostring(e),0)end end
local upgrading=fs.exists('/spawnnet-node/node.cfg');for p,d in pairs(FILES)do writef(p,d)end;writef('/spawnnet-node/version.txt','2.2.2\n')
local util=dofile('/spawnnet-node/util.lua');term.clear();term.setCursorPos(1,1);print('SpawnNet Storage Node 2.2.2');if upgrading then print('Existing node configuration preserved.');print('Pairing/token/object storage were NOT reset.');print();print('Run: spawnnet-node');print('Future updates: spawnnet-node update');return end
print('New node setup');local mods={};for _,n in ipairs(peripheral.getNames())do if peripheral.getType(n)=='modem'then mods[#mods+1]=n end end;local def=mods[1]or'';write('Modem ['..def..']: ');local modem=read();if modem==''then modem=def end;if modem==''then error('A modem is required',0)end;if not rednet.isOpen(modem)then rednet.open(modem)end;local discovery='spawnnet:discovery:v2';local nonce=util.id('discover');rednet.broadcast({network='spawnnet',version=2,type='discover',nonce=nonce},discovery);local timer=os.startTimer(1.5);local found={};while true do local ev={os.pullEvent()};if ev[1]=='timer'and ev[2]==timer then break end;if ev[1]=='rednet_message'then local sender,msg,proto=ev[2],ev[3],ev[4];if proto==discovery and type(msg)=='table'and msg.type=='advertise'and msg.nonce==nonce then found[msg.networkId]={id=msg.networkId,name=msg.name,coreId=sender};print('Found: '..msg.networkId..' - '..tostring(msg.name))end end end;local defaultId='public';for id in pairs(found)do defaultId=id;break end;write('Network ID ['..defaultId..']: ');local nid=util.safeName(read(),32);if nid==''then nid=defaultId end;local f=found[nid];local coreId=f and f.coreId or nil;if not coreId then write('Core computer ID: ');coreId=tonumber(read())end;if not coreId then error('Core ID required',0)end;local dn='Storage #'..os.getComputerID();write('Node name ['..dn..']: ');local name=read();if name==''then name=dn end;local pair=tostring(math.random(100000,999999));util.saveTable('/spawnnet-node/node.cfg',{networkId=nid,networkName=f and f.name or nid,coreId=coreId,name=name,modem=modem,pairCode=pair,token=nil});print();print('PAIR CODE: '..pair);print('Approve this computer with: spawnnet nodes');write('Install as startup? y/N: ');if read():lower()=='y'then if fs.exists('/startup.lua')and not fs.exists('/startup.pre-spawnnet-node.lua')then fs.copy('/startup.lua','/startup.pre-spawnnet-node.lua')end;writef('/startup.lua',"os.run({},'/spawnnet-node/node.lua')\n")end;print('Run now: spawnnet-node')

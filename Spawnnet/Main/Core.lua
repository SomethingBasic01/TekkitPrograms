-- SpawnNet 2.2.0 COMPLETE CORE
-- Fresh install OR in-place upgrade. No separate App Platform/hotfix installer.
-- Existing state, domains, accounts, sites, jobs, storage-node registry and network config are preserved.
local files={
...
ui.clear()
  ...
  ["/spawnnet/client/apps_forum.lua"]=[=[local net=dofile('/spawnnet/client/net.lua');local auth=dofile('/spawnnet/client/auth_client.lua');local ui=dofile('/spawnnet/client/ui.lua')
local sess=net.loadSession();if sess then local me=net.call('users','me',{});if not me then sess=nil end end;if not sess then local s,e=auth.ensureLogin();if not s then ui.error(e)return end end
local board='general'
while true do local p,e=net.call('forum','threads',{board=board});ui.clear('FORUM / '..board);if not p then ui.error(e);ui.pause();return end;for i,t in ipairs(p.threads)do print(i..') '..t.title..' ['..t.author..'] ('..t.replies..')')end;print();print('R # read   N new thread   B board   Q quit');write('> ');local line=read();local cmd,n=line:match('^(%a)%s*(%d*)');cmd=cmd and cmd:lower();if cmd=='q'then break elseif cmd=='b'then board=ui.prompt('Board',board)
elseif cmd=='n'then local title=ui.prompt('Title','');print('Body:');local body=read();local x,er=net.call('forum','newThread',{board=board,title=title,body=body});if not x then ui.error(er);ui.pause()end
elseif cmd=='r'then local t=p.threads[tonumber(n)or 0];if t then local x,er=net.call('forum','getThread',{board=board,id=t.id});if x then ui.clear(x.thread.title);print(x.thread.author..': '..x.thread.body);print('---');for _,r in ipairs(x.thread.replies)do print(r.author..': '..r.body)end;print();write('Reply? y/N ');if read():lower()=='y'then print('Reply:');net.call('forum','reply',{board=board,id=t.id,body=read()})end else ui.error(er);ui.pause()end end end end
ui.clear()
]=],
...
ui.clear()
  ...
  ["/spawnnet/client/apps_mail.lua"]=[=[local net=dofile('/spawnnet/client/net.lua');local auth=dofile('/spawnnet/client/auth_client.lua');local ui=dofile('/spawnnet/client/ui.lua')
local sess=net.loadSession();if sess then local me=net.call('users','me',{});if not me then sess=nil end end;if not sess then local s,e=auth.ensureLogin();if not s then ui.error(e)return end end
while true do
  local p,e=net.call('mail','inbox',{limit=60});ui.clear('SPAWNNET MAIL');if not p then ui.error(e);ui.pause();return end
  for i,m in ipairs(p.messages)do print(('%2d %s %-12s %s'):format(i,m.read and ' ' or '*',tostring(m.from):sub(1,12),tostring(m.subject):sub(1,28)))end
  print();print('R # read   C compose   D # delete   Q quit');write('> ');local line=read();local cmd,n=line:match('^(%a)%s*(%d*)');cmd=cmd and cmd:lower()
  if cmd=='q'then break elseif cmd=='c'then local to=ui.prompt('To','');local sub=ui.prompt('Subject','');print('Body (single line; use Studio/SDK for long formatted mail):');local body=read();local x,er=net.call('mail','send',{to=to,subject=sub,body=body});if x then ui.success('Sent')else ui.error(er)end;ui.pause()
  elseif cmd=='r'then local m=p.messages[tonumber(n)or 0];if m then net.call('mail','read',{id=m.id});ui.clear(m.subject);print('From: '..tostring(m.from));print('To: '..tostring(m.to));print();print(m.body);ui.pause()end
  elseif cmd=='d'then local m=p.messages[tonumber(n)or 0];if m then net.call('mail','delete',{id=m.id})end end
end
ui.clear()
]=],
...
local net=dofile('/spawnnet/client/net.lua')
local config=dofile('/spawnnet/lib/config.lua')
local function sessionName()local s=net.loadSession();return s and s.user or'Guest'end
local function run(path,...)shell.run(path,...)end
if net.loadSession()then pcall(net.call,'users','me',{})end
while true do
  local n=net.activeNetwork();local w,h=term.getSize();gui.clear();gui.bar('SPAWNNET '..config.version,(n.name or n.id)..' / '..sessionName())
  gui.text(2,3,w-2,'Websites, apps, machines, packages and private networks.',colors.lightGray,colors.black,'center')
  local buttons={
    {'WEB','/spawnnet/client/browser.lua','spn://home'},{'MAIL','/spawnnet/client/apps_mail.lua'},{'SEARCH','search'},
    {'STUDIO','/spawnnet/client/studio_easy.lua'},{'NETWORKS','/spawnnet/client/networks.lua'},{'APPS','/spawnnet/client/apps.lua'},
    {'LABS','/spawnnet/client/labs.lua'},{'DEV TOOLS','/spawnnet/client/developer.lua'},{'PACKAGES','/spawnnet-packages.lua'},
    {'MACHINES','/spawnnet/client/machines.lua'},{'DIAGNOSTICS','/spawnnet/client/doctor.lua'},{'SETTINGS','/spawnnet/client/settings.lua'},
  }
  local cols=3;local gap=2;local bw=math.max(12,math.floor((w-2-gap*(cols-1))/cols));local y0=6;local regions={}
  for i,b in ipairs(buttons)do local col=((i-1)%cols);local row=math.floor((i-1)/cols);local x=2+col*(bw+gap);local y=y0+row*3
    local r=gui.button(x,y,math.min(bw,w-x),b[1]);r.index=i;regions[#regions+1]=r
  end
  ...
  elseif ev[1]=='mouse_click'then local r=gui.hit(regions,ev[3],ev[4]);if r then local b=buttons[r.index]
    if b[2]=='search'then local q=gui.prompt('SEARCH','Search SpawnNet:','');if q~=''then run('/spawnnet/client/search.lua',q)end
...
local C=colors
local function clip(s,n)s=tostring(s or '');if #s>n then return s:sub(1,math.max(0,n-1))..'>' end;return s end
function M.clear(bg)
  term.setBackgroundColor(bg or C.black);term.setTextColor(C.white);term.clear();term.setCursorPos(1,1)
end
function M.text(x,y,w,text,fg,bg,align)
  local sw,sh=term.getSize();if y<1 or y>sh or x>sw then return end;w=math.max(0,math.min(w or #tostring(text),sw-x+1));if w<=0 then return end
  local s=clip(text,w);if align=='center'then s=string.rep(' ',math.max(0,math.floor((w-#s)/2)))..s elseif align=='right'then s=string.rep(' ',math.max(0,w-#s))..s end
  s=s..string.rep(' ',math.max(0,w-#s));term.setCursorPos(math.max(1,x),y);term.setTextColor(fg or C.white);term.setBackgroundColor(bg or C.black);write(s:sub(1,w))
end
function M.bar(title,right)
  local w=select(1,term.getSize());M.text(1,1,w,'',C.white,C.gray);M.text(2,1,math.max(1,w-3),title or 'SpawnNet',C.yellow,C.gray,'left')
  if right then M.text(math.max(2,w-#tostring(right)-1),1,#tostring(right),right,C.lightGray,C.gray,'right') end
end
function M.status(text,good)
  local w,h=term.getSize();M.text(1,h,w,' '..tostring(text or ''),good==false and C.red or C.lightGray,C.gray,'left')
end
function M.button(x,y,w,label,enabled)
  local bg=enabled==false and C.gray or C.blue;local fg=enabled==false and C.lightGray or C.white
  M.text(x,y,w,'['..tostring(label or '')..']',fg,bg,'center');return{x1=x,y1=y,x2=x+w-1,y2=y,label=label,enabled=enabled~=false}
end
function M.box(x,y,w,h,title,bg)
  bg=bg or C.black
  if w<2 or h<2 then return end
  M.text(x,y,w,string.rep('-',w),C.lightGray,bg)
  for yy=y+1,y+h-2 do M.text(x,yy,w,'|'..string.rep(' ',math.max(0,w-2))..'|',C.lightGray,bg) end
  M.text(x,y+h-1,w,string.rep('-',w),C.lightGray,bg)
  if title then M.text(x+2,y,math.min(w-4,#title),title,C.yellow,bg) end
end
function M.hit(regions,x,y)
  for i=#regions,1,-1 do local r=regions[i];if r.enabled~=false and x>=r.x1 and x<=r.x2 and y>=r.y1 and y<=r.y2 then return r end end
end
function M.prompt(title,label,default,mask)
  local w,h=term.getSize();M.clear();M.bar(title or 'INPUT');M.text(3,5,w-5,label or '',C.white,C.black);M.text(3,7,w-5,'> '..tostring(default or ''),C.black,C.lightGray)
  term.setCursorPos(5+#tostring(default or ''),7);term.setTextColor(C.black);term.setBackgroundColor(C.lightGray)
  local v=read(mask);if v==''and default~=nil then v=default end;term.setBackgroundColor(C.black);term.setTextColor(C.white);return v
end
function M.confirm(title,message)
  local w,h=term.getSize();M.clear();M.bar(title or 'CONFIRM');local y=5
  for line in (tostring(message or '')..'\n'):gmatch('(.-)\n')do M.text(3,y,w-5,line,C.white,C.black);y=y+1 end
  M.text(3,y+2,w-5,'Y = Yes    N = No',C.yellow,C.black)
  while true do local e,k=os.pullEvent('key');if k==keys.y then return true elseif k==keys.n or k==keys.q then return false end end
end
function M.menu(title,subtitle,items,opts)
  opts=opts or {};local selected=1
  while true do
    local w,h=term.getSize();M.clear(opts.bg or C.black);M.bar(title,opts.right)
    if subtitle then M.text(2,3,w-2,subtitle,C.lightGray,C.black,'center') end
    local start=opts.startY or 5;local regions={};local visible=math.max(1,h-start-2)
    ...
    for row=0,visible-1 do local i=first+row;if i>#items then break end;local it=items[i];local bg=i==selected and C.blue or C.black;local fg=it.disabled and C.gray or(i==selected and C.white or C.lightGray);M.text(3,start+row,w-5,(i==selected and '> 'or'  ')..tostring(it.label or it.id or i),fg,bg);regions[#regions+1]={x1=2,y1=start+row,x2=w-1,y2=start+row,index=i,enabled=not it.disabled}end
    M.status(opts.footer or 'Mouse: select/open   Up/Down/Enter   Q: back')
    local ev={os.pullEvent()};if ev[1]=='key'then local k=ev[2];if k==keys.up then selected=math.max(1,selected-1)elseif k==keys.down then selected=math.min(#items,selected+1)elseif k==keys.enter and items[selected]and not items[selected].disabled then return items[selected],selected elseif k==keys.q or k==keys.backspace then return nil end
...
function M.toast(message,seconds)
  local w,h=term.getSize();M.text(2,h-2,w-2,' '..tostring(message)..' ',C.black,C.yellow,'center');sleep(seconds or 1)
end
return M
]=],
...
local net=dofile('/spawnnet/client/net.lua')
local auth=dofile('/spawnnet/client/auth_client.lua')
local function inventoryPeripherals()
  local out={};for _,n in ipairs(peripheral.getNames())do local ok,list=pcall(peripheral.call,n,'list');if ok and type(list)=='table'then out[#out+1]=n end end;table.sort(out);return out
end
local function choosePeripheral(title,default)
  local names=inventoryPeripherals();local items={};for _,n in ipairs(names)do items[#items+1]={label=n..' ['..tostring(peripheral.getType(n))..']',name=n}end;items[#items+1]={label='Enter name manually',manual=true};items[#items+1]={label='Cancel',cancel=true};local m=gui.menu(title,'Inventory-capable peripherals detected: '..#names,items);if not m or m.cancel then return nil elseif m.manual then return gui.prompt(title,'Peripheral name:',default or '')else return m.name end
end
local function setupWarehouse()
  if not net.loadSession()then local u=gui.prompt('WAREHOUSE SETUP','SpawnNet username:','');local pw=gui.prompt('WAREHOUSE SETUP','Password:','','*');local s,e=auth.login(u,pw);if not s then gui.toast(e,2);return end end
  local mine,e=net.call('dns','listMine',{});if not mine then gui.toast(e,2);return end
  local sites={};for _,d in ipairs(mine.domains or {})do sites[#sites+1]={label=d.domain..' - '..d.title,domain=d.domain}end;if #sites==0 then gui.toast('Create a website first in Studio.',2);return end
  local sm=gui.menu('WAREHOUSE SETUP','Choose the site that owns this machine.',sites);if not sm then return end
  local source=choosePeripheral('SOURCE STORAGE');if not source then return end;local output=choosePeripheral('PICKUP / OUTPUT');if not output then return end
  local key,e=auth.createKey('Warehouse Agent # '..os.getComputerID(),{'jobs.*','telemetry.*'});if not key then gui.toast(e,2);return end
  util.ensureDir('/spawnnet/machines');local cfg={network=net.activeNetwork().id,domain=sm.domain,queue='warehouse',stream='warehouse',source=source,output=output,apiKeyId=key.id,apiKeySecret=key.secret,pollInterval=2,telemetryInterval=3};util.saveTable('/spawnnet/machines/warehouse.cfg',cfg)
  gui.clear();gui.bar('WAREHOUSE READY');gui.text(3,5,45,'Configured real inventory bridge:',colors.lime,colors.black);gui.text(3,7,45,source..'  ->  '..output,colors.white,colors.black);gui.text(3,9,45,'Site: spn://'..sm.domain,colors.white,colors.black);gui.text(3,11,45,'The API key was created automatically.',colors.lightGray,colors.black);gui.text(3,13,45,'Run: spawnnet machine run',colors.yellow,colors.black);gui.status('Press any key');os.pullEvent('key')
end
while true do
 local m=gui.menu('MACHINES','Connect real Minecraft peripherals to SpawnNet.',{{label='Set up Warehouse Agent',action='setup'},{label='Run Warehouse Agent',action='run'},{label='Edit warehouse config',action='edit'},{label='Telemetry Agent (generic)',action='telemetry'},{label='Back',action='back'}})
 if not m or m.action=='back'then break elseif m.action=='setup'then setupWarehouse()elseif m.action=='run'then shell.run('/spawnnet/client/warehouse_agent.lua')elseif m.action=='edit'then if fs.exists('/spawnnet/machines/warehouse.cfg')then shell.run('edit','/spawnnet/machines/warehouse.cfg')else gui.toast('Not configured yet',1)end elseif m.action=='telemetry'then shell.run('/spawnnet/client/telemetry_agent.lua')end
end
]=],
...
while true do
  ...
  ["/spawnnet/client/package_manager.lua"]=[=[-- SpawnNet native application package manager 2.2.0
local util=dofile('/spawnnet/lib/util.lua')
local net=dofile('/spawnnet/client/net.lua')
local crypto=dofile('/spawnnet/lib/crypto.lua')
...
local function ensureStartupBridge()
  local target='/startup.lua';local marker='SPAWNNET_SERVICE_BRIDGE_2.2.0';local current='';if fs.exists(target)then local h=fs.open(target,'r');if h then current=h.readAll();h.close()end end;if current:find(marker,1,true)then return true end;if current~=''and not fs.exists('/startup.pre-spawnnet.lua')then fs.copy(target,'/startup.pre-spawnnet.lua')end;local h=assert(fs.open(target,'w'));h.write("-- SPAWNNET_SERVICE_BRIDGE_2.2.0\nos.run({},'/spawnnet/client/startup_bridge.lua')\n");h.close();return true
end
...
gui.clear()
  ...
  ["/spawnnet/client/release_manager.lua"]=[=[-- SpawnNet release manager 2.2.0
...
local m=gui.menu('SEARCH: '..q,tostring(#(p.results or {}))..' result(s)',items);if m and m.domain then shell.run('/spawnnet/client/browser.lua','spn://'..m.domain)end
  ...
  ["/spawnnet/client/service_manager.lua"]=[=[-- SpawnNet application service manager 2.2.0
local util=dofile('/spawnnet/lib/util.lua')
...
local auth=dofile('/spawnnet/client/auth_client.lua')
while true do
  local s=net.loadSession();local n=net.activeNetwork()
  local items={{label='Network manager',action='net'},{label=s and('Log out '..s.user)or'Log in',action=s and'logout'or'login'},{label='Connection diagnostics',action='doctor'},{label='Check for client update',action='update'},{label='Back',action='back'}}
  local m=gui.menu('SPAWNNET SETTINGS','Active: '..tostring(n.name)..' ['..n.id..']',items)
  if not m or m.action=='back'then break elseif m.action=='net'then shell.run('/spawnnet/client/networks.lua')elseif m.action=='doctor'then shell.run('/spawnnet/client/doctor.lua')elseif m.action=='update'then shell.run('/spawnnet/client/updater.lua','client')elseif m.action=='logout'then auth.logout()elseif m.action=='login'then local u=gui.prompt('LOGIN','Username:','');local pw=gui.prompt('LOGIN','Password:','','*');local x,e=auth.login(u,pw);gui.toast(x and('Logged in as '..x.user)or e,2)end
end
]=],
...
elseif cmd=='machine'then if args[2]=='run'then shell.run('/spawnnet/client/warehouse_agent.lua')else shell.run('/spawnnet/client/machines.lua')end
...
elseif cmd=='logout'then auth.logout();print('Logged out.')
 ...
 print('  spawnnet studio          easy site builder')
 print('  spawnnet networks        network manager')
 print('  spawnnet machine         machine/peripheral bridge')
 ...
 print('  spawnnet update [check|rollback]  update client')
 print('  spawnnet packages        installed app manager')
 print('  spawnnet labs            showcase labs')
 print('  spawnnet dev             developer workbench')
end
]=],
  ["/spawnnet/client/startup_bridge.lua"]=[=[-- SpawnNet service startup bridge 2.2.0
local sm=dofile('/spawnnet/client/service_manager.lua')
...
local function login()
 ...
 local u=gui.prompt('STUDIO LOGIN','Username:','');local pw=gui.prompt('STUDIO LOGIN','Password:','','*');local s,e=auth.login(u,pw);if not s then gui.toast(e,2);return false end;return true
end
local function pageFor(title,tagline,body,kind)
 local bg=C.black;local accent=kind=='company'and C.blue or kind=='wiki'and C.purple or kind=='dashboard'and C.green or C.cyan
 return {title=title,background=bg,elements={
  {type='panel',x=1,y=1,w='100%',h=4,bg=C.gray,children={{type='heading',x=2,y=1,w=47,text=title,fg=C.yellow,bg=C.gray},{type='text',x=2,y=2,w=47,text=tagline,fg=C.white,bg=C.gray,align='center'}}},
  {type='badge',x=3,y=6,w=45,text=kind:upper()..' SITE',bg=accent,fg=C.white,align='center'},
  {type='text',x=3,y=9,w=45,h=6,text=body,fg=C.white,bg=C.black},
  {type='button',x=3,y=17,w=14,text='HOME',bg=accent,fg=C.white,action={type='navigate',target='/'}},
  {type='button',x=19,y=17,w=14,text='SEARCH',bg=C.gray,fg=C.white,action={type='search'}},
  {type='button',x=35,y=17,w=14,text='CONTACT',bg=C.gray,fg=C.white,action={type='navigate',target='/contact'}},
 }}
end
local function contactPage(title)
 return {title=title..' - Contact',background=C.black,elements={{type='heading',x=3,y=2,w=45,text='CONTACT '..title:upper(),fg=C.yellow},{type='text',x=3,y=5,w=45,h=4,text='Add your contact details here. Open Advanced Studio when you want forms, mail automation, APIs, machine controls or custom layouts.'},{type='button',x=3,y=11,w=16,text='< HOME',action={type='navigate',target='/'}}}}
end
local function createSite()
 ...
 local m=gui.menu('CHOOSE A STARTER','No Lua required.',{{label='Personal page',kind='personal'},{label='Company / organization',kind='company'},{label='Wiki / knowledge base',kind='wiki'},{label='Dashboard',kind='dashboard'},{label='Blank-ish starter',kind='blank'}});if not m then return end
 local tagline=gui.prompt('CREATE WEBSITE','Tagline:','Welcome to '..title)
 local body=gui.prompt('CREATE WEBSITE','Short intro:','This site was created with SpawnNet Studio. No Lua required.')
 local p,e=net.call('dns','register',{domain=domain,title=title});if not p then gui.toast(e,2);return end
 local home=pageFor(title,tagline,body,m.kind);local ok,er=net.call('web','savePage',{domain=domain,path='/',page=home});if not ok then gui.toast(er,2);return end
 net.call('web','savePage',{domain=domain,path='/contact',page=contactPage(title)})
 net.call('web','settings',{domain=domain,title=title,description=tagline,tags={m.kind,'spawnnet'}})
 local pub,pe=net.call('web','publish',{domain=domain,note='Created with Easy Studio'});if not pub then gui.toast(pe,2);return end
 gui.clear();gui.bar('WEBSITE PUBLISHED');gui.text(3,6,45,'Your website is live:',C.lime,C.black,'center');gui.text(3,8,45,'spn://'..domain,C.yellow,C.black,'center');gui.text(3,11,45,'You can keep using Easy Studio, or open Advanced Studio for every component/script/API.',C.lightGray,C.black,'center');gui.status('Press any key');os.pullEvent('key')
end
local function quickEdit(domain,site)
 local title=gui.prompt('QUICK EDIT','Site title:',site.title or domain);local tagline=gui.prompt('QUICK EDIT','Tagline:',site.description or '');local body=gui.prompt('QUICK EDIT','Home page text:','Welcome to '..title)
 local pg=pageFor(title,tagline,body,'personal');local p,e=net.call('web','savePage',{domain=domain,path='/',page=pg});if p then net.call('web','settings',{domain=domain,title=title,description=tagline});gui.toast('Draft saved - publish when ready.',1)else gui.toast(e,2)end
end
local function addPage(domain)
 local path=gui.prompt('ADD PAGE','Path:','/new-page');if path==''then return end;if path:sub(1,1)~='/'then path='/'..path end
 local title=gui.prompt('ADD PAGE','Page title:','New Page');local body=gui.prompt('ADD PAGE','Page text:','Write something awesome here.')
 local pg={title=title,background=C.black,elements={{type='heading',x=3,y=2,w=45,text=title,fg=C.yellow},{type='text',x=3,y=5,w=45,h=8,text=body},{type='button',x=3,y=15,w=14,text='< HOME',action={type='navigate',target='/'}}}}
 local p,e=net.call('web','savePage',{domain=domain,path=path,page=pg});gui.toast(p and'Draft page added' or e,2)
end

local function appendEvent(source,name,body)
  source=tostring(source or '')
  if source:find('event '..name,1,true)then return source end
  if source~=''and source:sub(-1)~='\n'then source=source..'\n'end
  return source..'\nevent '..name..'\n'..body..'\nend\n'
end
local function installContact(domain,site)
  local pg={title='Contact',background=C.black,elements={
    {type='heading',x=3,y=2,w=45,text='CONTACT US',fg=C.yellow},
    {type='text',x=3,y=5,w=45,h=2,text='This form sends persistent SpawnNet Mail to the site owner.'},
    {type='input',id='contactName',x=3,y=8,w=45,placeholder='Your name'},
    {type='input',id='contactMsg',x=3,y=10,w=45,placeholder='Message'},
    {type='button',x=3,y=13,w=22,text='SEND MESSAGE',bg=C.lime,fg=C.black,action={type='server',event='easy_contact_submit'}},
    {type='button',x=27,y=13,w=22,text='< HOME',action={type='navigate',target='/'}},
    {type='text',id='contactStatus',x=3,y=16,w=45,h=2,text='Ready.'}
  }}
  local script=appendEvent(site.serverScript,'easy_contact_submit',[=[  call site.owner -> owner
  call mail.send $owner "Website contact" "${input.contactName}: ${input.contactMsg}" -> mailid
  call ui.setText "contactStatus" "Sent! Mail ID: ${mailid}"
  call ui.alert "Message delivered through SpawnNet Mail"]=])
  local a,e=net.call('web','savePage',{domain=domain,path='/contact',page=pg});if not a then gui.toast(e,2);return end
  local b,er=net.call('web','saveScripts',{domain=domain,clientScript=site.clientScript or'',serverScript=script});gui.toast(b and'Contact form added to draft. Publish when ready.'or er,2)
end
local function installWarehouse(domain,site)
  local pg={title='Warehouse Control',background=C.black,liveInterval=2,liveServerEvent='easy_warehouse_refresh',elements={
    {type='heading',x=3,y=2,w=45,text='LIVE WAREHOUSE',fg=C.yellow},
    {type='badge',id='warehouseState',x=3,y=5,w=45,text='Checking machine...',bg=C.orange,fg=C.black,align='center'},
    {type='text',id='warehouseSummary',x=3,y=8,w=45,h=4,text='No telemetry yet.'},
    {type='input',id='item',x=3,y=13,w=29,value='minecraft:iron_ingot'},
    {type='input',id='count',x=34,y=13,w=15,value='16'},
    {type='button',x=3,y=15,w=45,text='EXPORT TO PICKUP INVENTORY',bg=C.lime,fg=C.black,action={type='server',event='easy_warehouse_export'}},
    {type='input',id='jobid',x=3,y=17,w=45,value='',placeholder='job id'},
    {type='text',id='jobState',x=3,y=19,w=45,h=2,text='No job submitted.'}
  }}
  local script=site.serverScript or''
  script=appendEvent(script,'easy_warehouse_refresh',[=[  call telemetry.last "warehouse" -> wh
  if $wh == nil
    call ui.setText "warehouseState" "WAREHOUSE OFFLINE"
    call ui.setText "warehouseSummary" "Run spawnnet machine on the inventory computer."
  else
    call ui.setText "warehouseState" "ONLINE - COMPUTER #${wh._computer}"
    call ui.setText "warehouseSummary" "${wh.summary}"
  endif
  if $input.jobid != ""
    call jobs.status $input.jobid -> j
    if $j != nil
      call ui.setText "jobState" "${j.status}: ${j.message} (${j.progress}%)"
    endif
  endif]=])
  script=appendEvent(script,'easy_warehouse_export',[=[  call user.isOwner -> allowed
  if $allowed == false
    call ui.alert "Owner login required to control physical inventory"
  else
    call jobs.submit "warehouse" "export" $input.item $input.count -> jid
    call ui.setValue "jobid" $jid
    call ui.setText "jobState" "QUEUED: ${jid}"
    call ui.alert "Physical machine job queued"
  endif]=])
  local a,e=net.call('web','savePage',{domain=domain,path='/warehouse',page=pg});if not a then gui.toast(e,2);return end
  local b,er=net.call('web','saveScripts',{domain=domain,clientScript=site.clientScript or'',serverScript=script});gui.toast(b and'Warehouse control added. Next: spawnnet machine, then Publish.'or er,2)
end
local function editSite(domain)
 while true do
  local got,e=net.call('web','getSite',{domain=domain});if not got then gui.toast(e,2);return end;local site=got.site;local pages=0;for _ in pairs((site.draft or {}).pages or {})do pages=pages+1 end
  local m=gui.menu('STUDIO: '..domain,tostring(site.title)..'  |  '..pages..' page(s)',{{label='Preview website',action='preview'},{label='Quick edit home page',action='edit'},{label='+ Add simple page',action='add'},{label='+ Contact form + Mail automation',action='contact'},{label='+ Live warehouse control (no Lua)',action='warehouse'},{label='Publish draft',action='publish'},{label='Advanced Studio / full designer',action='advanced'},{label='Back',action='back'}})
  if not m or m.action=='back'then return elseif m.action=='preview'then shell.run('/spawnnet/client/browser.lua','spn://'..domain)elseif m.action=='edit'then quickEdit(domain,site)elseif m.action=='add'then addPage(domain)elseif m.action=='contact'then installContact(domain,site)elseif m.action=='warehouse'then installWarehouse(domain,site)elseif m.action=='publish'then local note=gui.prompt('PUBLISH','Revision note:','Studio publish');local p,er=net.call('web','publish',{domain=domain,note=note});gui.toast(p and'Published!'or er,2)elseif m.action=='advanced'then shell.run('/spawnnet/client/studio_advanced.lua')end
 end
end
if not login()then return end
while true do
 local p,e=net.call('dns','listMine',{});if not p then gui.toast(e,2);return end;local items={{label='+ Create a website',action='create'}};for _,d in ipairs(p.domains or {})do items[#items+1]={label=d.domain..' - '..d.title,domain=d.domain}end;items[#items+1]={label='Back',action='back'}
 local m=gui.menu('SPAWNNET STUDIO','Basic websites require zero Lua. Advanced Studio is always one click away.',items)
...
return M
  ...
  ["/spawnnet/client/updater.lua"]=[=[-- SpawnNet transactional system updater 2.2.0
local net=dofile('/spawnnet/client/net.lua')
local util=dofile('/spawnnet/lib/util.lua')
local crypto=dofile('/spawnnet/lib/crypto.lua')
local arg=(...)or'client'
...
print('Backup: '..backup)
  ...
  ["/spawnnet/client/warehouse_agent.lua"]=[=[local util=dofile('/spawnnet/lib/util.lua')
local net=dofile('/spawnnet/client/net.lua')
local auth=dofile('/spawnnet/client/auth_client.lua')
local sn=dofile('/spawnnet/client/sdk.lua')
local cfgPath='/spawnnet/machines/warehouse.cfg'
local cfg=util.loadTable(cfgPath,nil)
if type(cfg)~='table'then printError('Warehouse agent is not configured. Run: spawnnet machine');return end
if cfg.network and net.registry().networks[cfg.network]then net.setActiveNetwork(cfg.network)end
if not peripheral.isPresent(cfg.source)then printError('Source peripheral missing: '..tostring(cfg.source));return end
if not peripheral.isPresent(cfg.output)then printError('Output peripheral missing: '..tostring(cfg.output));return end
if not net.loadSession()then
  local s,e
  if cfg.apiKeyId and cfg.apiKeySecret then s,e=auth.apiLogin(cfg.apiKeyId,cfg.apiKeySecret)
  else write('SpawnNet username: ');local u=read();write('Password: ');s,e=auth.login(u,read('*'))end
  if not s then printError(e);return end
end
local aliases={['minecraft:iron_ingot']='iron',['minecraft:gold_ingot']='gold',['minecraft:diamond']='diamond',['minecraft:redstone']='redstone',['minecraft:coal']='coal',['minecraft:copper_ingot']='copper'}
local function inventory()
  local ok,list=pcall(peripheral.call,cfg.source,'list');if not ok or type(list)~='table'then return nil,tostring(list)end
  local counts={};local used=0;local total=0
  for slot,item in pairs(list)do if type(item)=='table'and item.name then used=used+1;local c=tonumber(item.count)or 0;counts[item.name]=(counts[item.name]or 0)+c;total=total+c end end
  local data={online=true,source=cfg.source,output=cfg.output,usedSlots=used,totalItems=total,items=counts}
  for name,field in pairs(aliases)do data[field]=counts[name]or 0 end
  local top={};for name,count in pairs(counts)do top[#top+1]={name=name,count=count}end;table.sort(top,function(a,b)return a.count>b.count end)
  local parts={};for i=1,math.min(5,#top)do parts[#parts+1]=top[i].name:gsub('^minecraft:','')..' x'..top[i].count end;data.summary=table.concat(parts,', ')
  return data,list
end
local function pushTelemetry()
  local data,e=inventory();if not data then return nil,e end;local p,er=sn.telemetry.push(cfg.domain,cfg.stream or 'warehouse',data);return p,er
end
local function execute(j)
  local payload=j.payload or {};local action=j.action or '';if action~='export'then sn.jobs.fail(cfg.domain,j.id,'Unsupported action '..tostring(action));return end
  local item=tostring(payload.item or '');local want=math.max(1,math.floor(tonumber(payload.count)or 1));local claimed,e=sn.jobs.claim(cfg.domain,j.id,'warehouse-'..os.getComputerID());if not claimed then return end
  local data,list=inventory();if not data then sn.jobs.fail(cfg.domain,j.id,'Inventory read failed: '..tostring(list));return end
  local moved=0
  for slot,stack in pairs(list)do
    if stack.name==item and moved<want then
      local ask=math.min(want-moved,tonumber(stack.count)or 0)
      local ok,n=pcall(peripheral.call,cfg.source,'pushItems',cfg.output,slot,ask)
      if not ok then sn.jobs.fail(cfg.domain,j.id,'pushItems failed: '..tostring(n));return end
      moved=moved+(tonumber(n)or 0);sn.jobs.progress(cfg.domain,j.id,math.floor((moved/want)*100),'Moved '..moved..' / '..want)
    end
  end
  pushTelemetry()
  if moved>=want then sn.jobs.complete(cfg.domain,j.id,{item=item,count=moved,output=cfg.output},'Exported '..moved..' '..item)
  else sn.jobs.fail(cfg.domain,j.id,'Only moved '..moved..' of '..want..' '..item) end
end
print('SpawnNet Warehouse Agent 2.0')
print('Network: '..net.activeNetwork().name)
print('Site: spn://'..cfg.domain)
print('Source: '..cfg.source..' -> '..cfg.output)
print('Queue: '..tostring(cfg.queue or 'warehouse'))
print('Ctrl+T to stop.')
local lastTelemetry=0
while true do
  if os.clock()-lastTelemetry>=tonumber(cfg.telemetryInterval or 3)then local p,e=pushTelemetry();if not p then printError('telemetry: '..tostring(e))end;lastTelemetry=os.clock()end
  local p,e=sn.jobs.poll(cfg.domain,cfg.queue or 'warehouse',10);if p and p.jobs then for _,j in ipairs(p.jobs)do execute(j)end elseif e then printError('jobs: '..tostring(e))end
  sleep(tonumber(cfg.pollInterval)or 2)
end
]=],
...
return M
  ...
  ["/spawnnet/server/core_updater.lua"]=[=[-- SpawnNet Core staged updater - 2.2.0
local util=dofile('/spawnnet/lib/util.lua')
local crypto=dofile('/spawnnet/lib/crypto.lua')
...
function M.handle(state,req,ctx)
  local p=req.payload or {}; local a=req.action; stateLib.ensureUser(state,ctx.user)
  if a=='send' then local m,e=M.sendInternal(state,ctx.user,p.to,p.subject,p.body); if not m then return 404,nil,e end; return 201,{message=m},true
  elseif a=='inbox' then
    local out={}; local limit=math.min(tonumber(p.limit) or 50,100); for i=1,math.min(limit,#state.mail[ctx.user]) do out[i]=state.mail[ctx.user][i] end
    return 200,{messages=out},false
...
return M
  ...
  ["/spawnnet/server/services/package.lua"]=[=[-- SpawnNet package service - SpawnNet 2.2.0
local util=dofile('/spawnnet/lib/util.lua')
local C=dofile('/spawnnet/server/services/common.lua')
local auth=dofile('/spawnnet/server/auth.lua')
local crypto=dofile('/spawnnet/lib/crypto.lua')
...
function M.handle(state,req,ctx)
  ...
  if a=='manifest'then local name=cleanName(p.name or'client');local pkg=loadSystem(state,name);if not pkg then return 404,nil,'Package not found'end;return 200,{package=systemManifest(pkg),name=name,version=pkg.version,description=pkg.description,files=count(pkg.files)},false end
  if a=='get'then local name=cleanName(p.name or'client');local pkg=loadSystem(state,name);if not pkg then return 404,nil,'Package not found'end;return 200,{package=util.deepcopy(pkg)},false end
...
return M
  ...
  ["/spawnnet-node/node.lua"]=[=[local VERSION='2.2.0';local ROOT='/spawnnet-node';local cfgPath=ROOT..'/node.cfg';local util=dofile(ROOT..'/util.lua');local wire=dofile(ROOT..'/wire.lua');local cfg=util.loadTable(cfgPath,nil);if type(cfg)~='table'then error('Missing '..cfgPath)end;local discovery='spawnnet:discovery:v2';local backbone='spawnnet:backbone:'..cfg.networkId..':v2';local fragments={}
...
local function discoverCore()local nonce=util.id('node-discover');rednet.broadcast({network='spawnnet',version=2,type='discover',nonce=nonce},discovery);local timer=os.startTimer(1.5);while true do local ev={os.pullEvent()};if ev[1]=='timer'and ev[2]==timer then return nil end;if ev[1]=='rednet_message'then local sender,msg,proto=ev[2],ev[3],ev[4];if proto==discovery and type(msg)=='table'and msg.type=='advertise'and msg.nonce==nonce and msg.networkId==cfg.networkId then cfg.coreId=sender;cfg.networkName=msg.name;util.saveTable(cfgPath,cfg);return sender end end end end
if not cfg.coreId then discoverCore()end
local function hello()if not cfg.coreId then if not discoverCore()then return end end;local free,cap,count=stats();rednet.send(cfg.coreId,{network='spawnnet',version=2,type='node_hello',networkId=cfg.networkId,pairCode=cfg.pairCode,name=cfg.name,free=free,capacity=cap,objects=count,nodeVersion=VERSION},backbone)end
local function heartbeat()if not cfg.coreId or not cfg.token then return end;local free,cap,count=stats();rednet.send(cfg.coreId,{network='spawnnet',version=2,type='node_heartbeat',networkId=cfg.networkId,token=cfg.token,name=cfg.name,free=free,capacity=cap,objects=count,nodeVersion=VERSION},backbone)end
local function respond(to,req,ok,payload,err)local r={network='spawnnet',version=2,type='cluster_response',networkId=cfg.networkId,requestId=req.requestId,ok=ok~=false,error=err};for k,v in pairs(payload or{})do r[k]=v end;wire.send(to,r,backbone)end
local function handle(sender,msg)if sender~=cfg.coreId or type(msg)~='table'or msg.networkId~=cfg.networkId or msg.token~=cfg.token then return end;local op=msg.op;local id=tostring(msg.objectId or'');if id==''then respond(sender,msg,false,nil,'bad object id');return end;local path=objectPath(id);if op=='put'then local data=tostring(msg.data or'');local ok,e=pcall(util.writeFile,path,data);if not ok then respond(sender,msg,false,nil,tostring(e));return end;local free=stats();respond(sender,msg,true,{size=#data,free=free})elseif op=='get'then local data=util.readFile(path);if data==nil then respond(sender,msg,false,nil,'object missing')else respond(sender,msg,true,{data=data,size=#data})end elseif op=='delete'then if fs.exists(path)then fs.delete(path)end;respond(sender,msg,true,{ok=true})elseif op=='ping'then local free,cap,count=stats();respond(sender,msg,true,{free=free,capacity=cap,objects=count})else respond(sender,msg,false,nil,'unknown op')end end
term.clear();term.setCursorPos(1,1);print('SpawnNet Storage Node '..VERSION);print('Network: '..tostring(cfg.networkName or cfg.networkId)..' ['..cfg.networkId..']');print('Node: #'..os.getComputerID()..' '..tostring(cfg.name));print(cfg.token and'PAIRED / ONLINE'or'WAITING FOR ADMIN APPROVAL');print();print('Update later: spawnnet-node update');print('Ctrl+T stops node.');hello();local timer=os.startTimer(cfg.token and 10 or 4);while true do local ev={os.pullEvent()};if ev[1]=='timer'and ev[2]==timer then if cfg.token then heartbeat()else hello()end;timer=os.startTimer(cfg.token and 10 or 4)elseif ev[1]=='rednet_message'then local sender,msg,proto=ev[2],ev[3],ev[4];if proto==backbone then if type(msg)=='table'and msg.type=='node_approved'and sender==cfg.coreId and msg.networkId==cfg.networkId then cfg.token=msg.token;cfg.name=msg.name or cfg.name;util.saveTable(cfgPath,cfg);print('APPROVED! Node is now active.');heartbeat()elseif cfg.token then local complete=wire.accept(sender,msg,fragments);wire.purge(fragments);if complete then handle(sender,complete)end end end end end
]=],
  ["/spawnnet-node/updater.lua"]=[=[-- SpawnNet Storage Node updater 2.2.0
...
local function request(kind)
  ...
  local ok,e=wire.send(cfg.coreId,{network='spawnnet',version=2,type=kind,networkId=cfg.networkId,token=cfg.token,requestId=id},proto)
  if not ok then return nil,e end
  ...
  while true do
    ...
    if ev[1]=='rednet_message'and ev[2]==cfg.coreId and ev[4]==proto then
      ...
      if type(body)=='table'and body.type=='node_update_response'and body.requestId==id and body.token==cfg.token then
...
local localv=(util.readFile(ROOT..'/version.txt')or'2.2.0'):gsub('%s+$','')
...
local function validateMap(map,label)
  local count=0
  for path,data in pairs(map)do
    if path:sub(-4)=='.lua'then
      local f,e=loadstring(data,'@'..path)
      if not f then error('REFUSING INSTALL: '..label..' '..path..' has a Lua syntax error: '..tostring(e),0)end
      count=count+1
    end
  end
  return count
...
local function count(t)local n=0;for _ in pairs(t or{})do n=n+1 end;return n end
term.clear();term.setCursorPos(1,1);term.setTextColor(colors.yellow);print('SPAWNNET CORE '..VERSION);term.setTextColor(colors.white)
local upgrading=fs.exists('/spawnnet/server/server.lua')
print(upgrading and'Complete in-place Core upgrade detected.'or'Fresh complete Core install.')
print('Validating code BEFORE touching the live installation...')
local a=validateMap(files,'Core');local b=validateMap(nodeReleaseFiles,'Node release');print('Lua validation: '..a..' Core/Client + '..b..' Node files OK')
...
local function firstAdmin()
  if serverCfg.adminUser and state.accounts[serverCfg.adminUser]and hasRole(state.accounts[serverCfg.adminUser],'admin')then return serverCfg.adminUser end
  for u,acc in pairs(state.accounts or{})do if hasRole(acc,'admin')then return u end end
end
local function createOrResetAdmin(user,password)
  user=util.safeName(user,24)
  if #user<3 or #password<4 then error('Username/password too short',0)end
  local salt=crypto.randomHex(12);local verifier=crypto.sha256(user..':'..password..':'..salt);local acc=state.accounts[user]
  if not acc then acc={username=user,verifier=verifier,salt=salt,created=util.now(),roles={'admin'},profile={displayName=user,bio=''}};state.accounts[user]=acc
  else acc.verifier=verifier;acc.salt=salt;acc.roles=acc.roles or{};if not hasRole(acc,'admin')then acc.roles[#acc.roles+1]='admin'end end
  stateLib.ensureUser(state,user);return user
end
local admin=firstAdmin()
if not upgrading or count(state.accounts)==0 then
  if count(state.accounts)==0 then
    print();print('FRESH NETWORK SETUP')
    write('Admin username: ');local u=read();write('Admin password: ');admin=createOrResetAdmin(u,read('*'))
  elseif not admin then
    local first=nil;for u in pairs(state.accounts)do first=u;break end
    write('Promote existing account ['..tostring(first or'')..']: ');local c=util.safeName(read(),24);if c==''then c=first end
    if not c or not state.accounts[c]then error('Choose an existing account',0)end
    state.accounts[c].roles=state.accounts[c].roles or{};state.accounts[c].roles[#state.accounts[c].roles+1]='admin';admin=c
  end
  print();print('NETWORK IDENTITY')
  local defaultId=util.safeName(serverCfg.networkId or'public',32);if defaultId==''then defaultId='public'end
  ...
  if vis=='private'then write('Private join code '..(joinHash~=''and'[keep existing if blank]'or'[required]')..': ');local code=read('*');if code~=''then joinHash=crypto.sha256(code)elseif joinHash==''then error('Private networks require a join code',0)end else joinHash=''end
  serverCfg.networkId=nid;serverCfg.networkName=nn:sub(1,48);serverCfg.visibility=vis;serverCfg.joinHash=joinHash
else
  print('Preserving existing network/state:')
  print('  Network: '..tostring(serverCfg.networkName)..' ['..tostring(serverCfg.networkId)..']')
  print('  Accounts: '..count(state.accounts)..'  Domains: '..count(state.domains))
  print('  Sites/jobs/nodes/storage data preserved')
  if not admin then
    for u in pairs(state.accounts or{})do admin=u;break end
    if admin then state.accounts[admin].roles=state.accounts[admin].roles or{};state.accounts[admin].roles[#state.accounts[admin].roles+1]='admin'end
  end
end
local mods={};for _,n in ipairs(peripheral.getNames())do if peripheral.getType(n)=='modem'then mods[#mods+1]=n end end
if not(serverCfg.modem and peripheral.isPresent(serverCfg.modem)and peripheral.getType(serverCfg.modem)=='modem')then
  local def=mods[1]or'';print();write('Core modem ['..def..']: ');local m=read();if m==''then m=def end;if m==''then error('A modem is required',0)end;serverCfg.modem=m
end
serverCfg.adminUser=admin
serverCfg.networkId=util.safeName(serverCfg.networkId or'public',32);if serverCfg.networkId==''then serverCfg.networkId='public'end
serverCfg.networkName=tostring(serverCfg.networkName or serverCfg.networkId):sub(1,48)
serverCfg.visibility=serverCfg.visibility=='private'and'private'or'public'
serverCfg.protocol=config.protocolPrefix..serverCfg.networkId..':v2';serverCfg.backboneProtocol=config.backbonePrefix..serverCfg.networkId..':v2';serverCfg.log=serverCfg.log or'/spawnnet/log/server.log'
util.saveTable(config.serverConfig,serverCfg)
state.meta=state.meta or{};state.meta.networkId=serverCfg.networkId;state.meta.networkName=serverCfg.networkName
stateLib.seedSystem(state,admin or'system')
state.packages=state.packages or{}
state.packages.client={version=VERSION,description='Official SpawnNet complete client',owner=admin,published=util.now(),component='client',channel='stable'}
state.packages.node={version=VERSION,description='Official SpawnNet storage node',owner=admin,published=util.now(),component='node',channel='stable'}
...
local function release(name,component,description,map,selector,restart)
  local rf={};local total=0
  for path,data in pairs(map)do
    local rel=path:gsub('^/','')
    if selector(rel)then rf[rel]=data;total=total+#data end
  end
  local pkg={name=name,version=VERSION,description=description,files=rf,totalBytes=total,owner=admin,component=component,channel='stable',restartRequired=restart and true or false,published=util.now(),updated=util.now()}
  pkg.hash=systemHash(pkg);ensure('/spawnnet/releases');writeFile('/spawnnet/releases/'..name..'.pkg',textutils.serialize(pkg));return pkg
end
release('client','client','Official SpawnNet '..VERSION..' client',files,function(p)return p:sub(1,16)=='spawnnet/client/'or p:sub(1,13)=='spawnnet/lib/'or p=='spawnnet.lua'or p=='web.lua'or p=='studio.lua'or p=='spawnnet-packages.lua'or p=='spawnnet-release.lua'end,false)
release('node','node','Official SpawnNet '..VERSION..' storage node',nodeReleaseFiles,function(p)return p:sub(1,14)=='spawnnet-node/'or p=='spawnnet-node.lua'end,true)
local okSave,saveErr=pcall(stateLib.save,state,config.dataFile);if not okSave then error('SpawnNet state save failed: '..tostring(saveErr),0)end
ensure('/spawnnet/version');writeFile('/spawnnet/version/core-release.txt',VERSION..'\n')
for _,p in ipairs({'/SpawnNet-App-Platform-Core-1.0.0.lua','/SpawnNet-App-Platform-Core-1.0.1.lua','/corehotfixV1.txt'})do if fs.exists(p)then pcall(fs.delete,p)end end
local startupAlready=false
if fs.exists('/startup.lua')then local h=fs.open('/startup.lua','r');local s=h and h.readAll()or'';if h then h.close()end;startupAlready=s:find('/spawnnet/server/startup.lua',1,true)~=nil end
if startupAlready then writeFile('/startup.lua',"shell.run('/spawnnet/server/startup.lua')\n")
elseif not upgrading then
  write('Install Core as /startup.lua? y/N: ')
  if read():lower()=='y'then
    if fs.exists('/startup.lua')and not fs.exists('/startup.pre-spawnnet.lua')then fs.copy('/startup.lua','/startup.pre-spawnnet.lua')end
    writeFile('/startup.lua',"shell.run('/spawnnet/server/startup.lua')\n")
  end
end
print();term.setTextColor(colors.lime);print(upgrading and'CORE UPGRADE COMPLETE'or'CORE INSTALL COMPLETE');term.setTextColor(colors.white)
print('Network: '..serverCfg.networkName..' ['..serverCfg.networkId..']')
print('Run: spawnnet-server')
print('Status: spawnnet-status')
print('Future Core versions: run the NEW complete Core installer over this one.')
print('Clients can update from this Core with: spawnnet update')
print('Nodes can update from this Core with: spawnnet-node update')

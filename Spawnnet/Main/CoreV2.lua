...
while true do
  ...
  ["/spawnnet/client/package_manager.lua"]=[=[-- SpawnNet native application package manager 2.2.2
local util=dofile('/spawnnet/lib/util.lua')
local net=dofile('/spawnnet/client/net.lua')
local crypto=dofile('/spawnnet/lib/crypto.lua')
...
local function ensureStartupBridge()
  local target='/startup.lua';local marker='SPAWNNET_SERVICE_BRIDGE_2.2.2';local current=''
  if fs.exists(target)then local h=fs.open(target,'r');if h then current=h.readAll();h.close()end end
  if current:find(marker,1,true)then return true end
  local oldBridge=current:find('SPAWNNET_SERVICE_BRIDGE_',1,true)~=nil
  if current~=''and not oldBridge and not fs.exists('/startup.pre-spawnnet.lua')then fs.copy(target,'/startup.pre-spawnnet.lua')end
  local h=assert(fs.open(target,'w'));h.write("-- SPAWNNET_SERVICE_BRIDGE_2.2.2\nif not shell or not shell.run then error('SpawnNet service bridge requires CraftOS shell',0) end\nshell.run('/spawnnet/client/startup_bridge.lua')\n");h.close();return true
end
...
gui.clear()
  ...
  ["/spawnnet/client/release_manager.lua"]=[=[-- SpawnNet release manager 2.2.2
...
local m=gui.menu('SEARCH: '..q,tostring(#(p.results or {}))..' result(s)',items);if m and m.domain then shell.run('/spawnnet/client/browser.lua','spn://'..m.domain)end
  ...
  ["/spawnnet/client/service_manager.lua"]=[=[-- SpawnNet application service manager 2.2.2
...
local function signature(t)
  local a={};for k,s in pairs(t or{})do a[#a+1]=tostring(k)..'|'..tostring(s.enabled)..'|'..tostring(s.path)end;table.sort(a);return table.concat(a,'\n')
end
local function runOne(s)
  while true do
    if not(s and s.enabled and s.path and fs.exists(s.path))then return end
    local env={};if shell then env.shell=shell end
    local ok,err=pcall(function()local ran=os.run(env,s.path,'spawnnet-service');if not ran then error('service program exited with failure',0)end end)
...
function M.runAll()
  while true do
    ...
    for _,s in pairs(t)do if s.enabled and s.path then funcs[#funcs+1]=function()runOne(s)end end end
...
elseif cmd=='logout'then auth.logout();print('Logged out.')
 ...
 print('  spawnnet dev                   developer workbench')
  ...
  ["/spawnnet/client/startup_bridge.lua"]=[=[-- SpawnNet service startup bridge 2.2.2
...
return M
  ...
  ["/spawnnet/client/updater.lua"]=[=[-- SpawnNet transactional system updater 2.2.2
local net=dofile('/spawnnet/client/net.lua')
local util=dofile('/spawnnet/lib/util.lua')
local crypto=dofile('/spawnnet/lib/crypto.lua')
local arg=(...)or'client'
...
return M
  ...
  ["/spawnnet/server/core_updater.lua"]=[=[-- SpawnNet Core staged updater - 2.2.2
local util=dofile('/spawnnet/lib/util.lua')
local crypto=dofile('/spawnnet/lib/crypto.lua')
...
return M
  ...
  ["/spawnnet/server/services/package.lua"]=[=[-- SpawnNet package service - SpawnNet 2.2.2
local util=dofile('/spawnnet/lib/util.lua')
local C=dofile('/spawnnet/server/services/common.lua')
local auth=dofile('/spawnnet/server/auth.lua')
local crypto=dofile('/spawnnet/lib/crypto.lua')
...
return M
  ...
  ["/spawnnet-node/node.lua"]=[=[local VERSION='2.2.2';local ROOT='/spawnnet-node';local cfgPath=ROOT..'/node.cfg';local util=dofile(ROOT..'/util.lua');local wire=dofile(ROOT..'/wire.lua');local cfg=util.loadTable(cfgPath,nil);if type(cfg)~='table'then error('Missing '..cfgPath)end;local discovery='spawnnet:discovery:v2';local backbone='spawnnet:backbone:'..cfg.networkId..':v2';local fragments={}
...
term.clear();term.setCursorPos(1,1);print('SpawnNet Storage Node '..VERSION);print('Network: '..tostring(cfg.networkName or cfg.networkId)..' ['..cfg.networkId..']');print('Node: #'..os.getComputerID()..' '..tostring(cfg.name));print(cfg.token and'PAIRED / ONLINE'or'WAITING FOR ADMIN APPROVAL');print();print('Update later: spawnnet-node update');print('Ctrl+T stops node.');hello();local timer=os.startTimer(cfg.token and 10 or 4);while true do local ev={os.pullEvent()};if ev[1]=='timer'and ev[2]==timer then if cfg.token then heartbeat()else hello()end;timer=os.startTimer(cfg.token and 10 or 4)elseif ev[1]=='rednet_message'then local sender,msg,proto=ev[2],ev[3],ev[4];if proto==backbone then if type(msg)=='table'and msg.type=='node_approved'and sender==cfg.coreId and msg.networkId==cfg.networkId then cfg.token=msg.token;cfg.name=msg.name or cfg.name;util.saveTable(cfgPath,cfg);print('APPROVED! Node is now active.');heartbeat()elseif cfg.token then local complete=wire.accept(sender,msg,fragments);wire.purge(fragments);if complete then handle(sender,complete)end end end end end
  ...
  ["/spawnnet-node/updater.lua"]=[=[-- SpawnNet Storage Node updater 2.2.2
...
local localv=(util.readFile(ROOT..'/version.txt')or'2.2.2'):gsub('%s+$','')
...
for _,p in ipairs({'/SpawnNet-App-Platform-Core-1.0.0.lua','/SpawnNet-App-Platform-Core-1.0.1.lua','/corehotfixV1.txt','/spawnnet/client/warehouse_agent.lua','/spawnnet/machines/warehouse.cfg'})do if fs.exists(p)then pcall(fs.delete,p)end end

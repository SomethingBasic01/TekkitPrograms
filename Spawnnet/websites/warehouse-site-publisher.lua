-- WarehouseOS 1.0.0 SpawnNet Publisher
...
local function nav(p,y,w)
  add(p,{type='button',x=3,y=y,w=10,text='HOME',bg=C.gray,action={type='navigate',target='/'}})
  add(p,{type='button',x=14,y=y,w=12,text='MY STORAGE',bg=C.blue,action={type='navigate',target='/my'}})
  add(p,{type='button',x=27,y=y,w=10,text='POCKET',bg=C.purple,action={type='navigate',target='/pocket'}})
  add(p,{type='button',x=38,y=y,w=11,text='HELP',bg=C.gray,action={type='navigate',target='/help'}})
end
...
add(p,{type='button',x=27,y=21,w=22,text='CREATE WAREHOUSE',bg=C.purple,action={type='navigate',target='/create'}})
add(p,{type='button',x=3,y=23,w=46,text='OPEN MY WAREHOUSES',bg=C.blue,action={type='navigate',target='/my'}})
...
local function terminalPage(pocket)
  ...
  for i=1,8 do hidden(pg,'item'..i,'');hidden(pg,'itemName'..i,'');hidden(pg,'itemAmount'..i,'')end
  ...
  if not pocket then
    add(pg,{type='button',x=3,y=y,w=14,text='MEMBERS',bg=C.purple,action={type='navigate',target='/members'}});add(pg,{type='button',x=18,y=y,w=14,text='HISTORY',bg=C.gray,action={type='navigate',target='/history'}});add(pg,{type='button',x=33,y=y,w=16,text='SETTINGS',bg=C.gray,action={type='navigate',target='/settings'}})
  else add(pg,{type='button',x=x,y=y,w=width,text='FULL TERMINAL',bg=C.gray,action={type='navigate',target='/terminal'}})end
  return pg
end
...
term.clear();term.setCursorPos(1,1);term.setTextColor(C.lime);print('WAREHOUSEOS PUBLISHER 1.0.0');term.setTextColor(C.white)
...
write('Publishing... ');call('web','publish',{domain=DOMAIN,note='WarehouseOS 1.0.0 initial release'});print('OK')

...
if not serviceMode then
  local ok,msg=common.startBackground('/warehouseos/controller.lua','/warehouseos/controller.running','service')
  gui.toast(ok and'WarehouseOS Controller is running in the background.'or msg,3)
  return
end
...
local state=util.loadTable(DB,{profiles={},warehouses={},pairCodes={},terminalCodes={},pending={},pendingCompletions={},stats={jobs=0,withdrawals=0,deposits=0}})
state.profiles=state.profiles or{};state.warehouses=state.warehouses or{};state.pendingCompletions=state.pendingCompletions or{};state.stats=state.stats or{jobs=0,withdrawals=0,deposits=0}
...
local function immediateJob(j)
  ...
  elseif action=='create'then
    -- Website 1.2.0 sends the human-facing name in note, keeping payload.item fixed.
...
while running do
  ...
  elseif ev[1]=='timer'and ev[2]==timer then
    wire.purge();pollJobs();retryCompletions();local now=os.clock()
...
function M.enableMultishell()
  if not(settings and settings.set)then return false,'settings API unavailable'end
  local okSet=pcall(settings.set,'bios.use_multishell',true)
  if not okSet then return false,'could not set bios.use_multishell'end
  if settings.save then
    -- ComputerCraft/CraftOS 1.8 requires an explicit settings filename.
    -- Calling settings.save() with no path does NOT persist the value on this build.
    local ok,res=pcall(settings.save,'.settings')
    if not ok or res==false then ok,res=pcall(settings.save,'/.settings')end
    if not ok or res==false then return false,'could not save /.settings'end
  else
    return false,'settings.save unavailable'
  end
  return true
end
function M.startBackground(path,lock,...)
  ...
  if advanced then
    local saved,why=M.enableMultishell()
    if saved then return false,'Advanced Computer detected. WarehouseOS repaired and SAVED bios.use_multishell=true to /.settings. Reboot ONCE, then run this command again.'end
    return false,'Advanced Computer detected, but WarehouseOS could not persist multishell: '..tostring(why)
  end
...
if not serviceMode then local ok,msg=common.startBackground('/warehouseos/terminal.lua','/warehouseos/terminal.running','service');gui.toast(ok and'WarehouseOS Remote Terminal is running in the background.'or msg,3);return end
...
local t=os.startTimer(1);local last=0;draw();while true do local ev={os.pullEvent()};if ev[1]=='rednet_message'and ev[2]==cfg.host and ev[4]==protocol then local obj,done=wire.accept(ev[2],ev[3]);if done and obj then handle(obj);draw()end elseif ev[1]=='timer'and ev[2]==t then local now=os.clock();common.writeRunning('/warehouseos/terminal.running');for id,r in pairs(pendingResults)do if now-r.created>12 then pendingResults[id]=nil elseif now-r.last>=1 then r.last=now;wire.send(cfg.host,r.msg,protocol)end end;if now-last>4 then last=now;heartbeat()end;draw();t=os.startTimer(1)elseif ev[1]=='key'and(ev[2]==keys.q or ev[2]==keys.escape)then break end end;gui.clear()
]=],
  ["/warehouseos/autostart.lua"]=[=[local common=dofile('/warehouseos/lib/common.lua')
if fs.exists('/warehouseos/controller.db')then common.startBackground('/warehouseos/controller.lua','/warehouseos/controller.running','service')end
if fs.exists('/warehouseos/terminal.db')then common.startBackground('/warehouseos/terminal.lua','/warehouseos/terminal.running','service')end
]=],
}
local function mkdirFor(path) local d=fs.getDir(path);if d~=''and not fs.exists(d)then fs.makeDir(d)end end
term.clear();term.setCursorPos(1,1);term.setTextColor(colors.lime);print('WAREHOUSEOS 1.2.0');term.setTextColor(colors.white);print('Installing ME-style storage tools for SpawnNet...');print()
...
local multishellSaveError=nil
local advanced=(term.isColor and term.isColor())or(term.isColour and term.isColour())or false
local hasTabs=(shell and shell.openTab)or(multishell and multishell.launch)
if advanced and not hasTabs then
  local common=dofile('/warehouseos/lib/common.lua')
  local ok,why=common.enableMultishell()
  multishellWasEnabled=ok
  multishellSaveError=ok and nil or why
end
local function installStartupHook()
  ...
  local path='/startup';local begin='-- WarehouseOS autostart begin';local finish='-- WarehouseOS autostart end'
  local old='';if fs.exists(path)then local h=fs.open(path,'r');if h then old=h.readAll();h.close()end end
  if not old:find(begin,1,true)then
    local h=fs.open(path,'w');if h then h.write(old..(old~=''and'\n'or'')..begin.."\nif fs.exists('/warehouseos/autostart.lua') then shell.run('/warehouseos/autostart.lua') end\n"..finish..'\n');h.close()end
  end
end
installStartupHook()
print();term.setTextColor(colors.lime);print('INSTALL COMPLETE');term.setTextColor(colors.white)
if multishellWasEnabled then term.setTextColor(colors.yellow);print();print('ADVANCED COMPUTER DETECTED.');print('WarehouseOS repaired bios.use_multishell and SAVED it to /.settings.');print('Reboot this computer ONCE. After reboot background services will work.');term.setTextColor(colors.white)
elseif multishellSaveError then term.setTextColor(colors.red);print();print('MULTISHELL REPAIR FAILED: '..tostring(multishellSaveError));print('Run: set bios.use_multishell true');term.setTextColor(colors.white)end
print();print('Commands:');print('  warehouse             WarehouseOS menu');print('  warehouse-host        Central always-loaded host');print('  warehouse-controller  Physical warehouse controller');print('  warehouse-terminal    Wireless remote terminal');print();print('Publish the website separately with WarehouseOS-Publisher-1.2.0.lua')

-- WarehouseOS 1.3.0 SpawnNet Publisher
...
-- Create
p=base('Create Warehouse','Create the virtual warehouse, then pair the physical controller','create');nav(p,5);hidden(p,'createToken','')
...
add(p,{type='text',x=3,y=31,w=45,h=5,text='Next: run warehouse-controller on the inventory computer and enter the pair code. If an older creation ever lost its displayed code, open that warehouse in My Warehouses -> Settings and generate a fresh Controller Pair Code.'})
pages['/create']=p
...
add(p,{type='heading',x=3,y=43,w=45,text='DANGER ZONE',fg=C.red})
add(p,{type='text',x=3,y=45,w=45,h=3,text='Deleting a warehouse removes its WarehouseOS directory, aliases, members, terminal registrations and controller pairing. Physical items in chests are NOT deleted.'})
add(p,{type='input',id='deleteConfirm',x=3,y=49,w=45,value='',placeholder='Type DELETE to confirm'})
add(p,{type='button',x=3,y=52,w=45,text='DELETE THIS WAREHOUSE',bg=C.red,fg=C.white,action={type='server',event='delete_warehouse_submit'}})
add(p,{type='text',id='deleteStatus',x=3,y=54,w=45,h=2,text='Owner only. This cannot be undone.'})
pages['/settings']=p
...
event create_submit
  ...
  if $who == nil
    ...
    random createToken 100000 999999
    concat createReq "warehouse|" $createToken
    call ui.setValue "createToken" $createToken
    call jobs.submit "host" "create" $createReq 0 $requestedName -> jid
    call ui.setValue "jobId" $jid
    call ui.setText "createStatus" "CREATING: ${requestedName}"
    call ui.setText "createId" "Warehouse ID: waiting for Host..."
    call ui.setText "createPair" "Controller pair code: waiting for Host..."
  endif
end
...
event delete_warehouse_submit
  if $input.deleteConfirm != "DELETE"
    call ui.alert "Type DELETE exactly to confirm warehouse deletion."
  else
    concat req $input.warehouseId "|DELETE"
    call jobs.submit "host" "delete_warehouse" $req 0 "" -> jid
    call ui.setValue "jobId" $jid
    call ui.setText "deleteStatus" "Deleting warehouse..."
  endif
end

event poll_job
  if $input.createToken != ""
    call user.name -> createUser
    if $createUser != nil
      concat createStream "create-" $createUser "-" $input.createToken
      call telemetry.last $createStream -> created
      if $created != nil
        call ui.setText "createStatus" "WAREHOUSE CREATED"
        call ui.setText "createId" "Warehouse ID: ${created.warehouseId}"
        call ui.setText "createPair" "Controller pair code: ${created.pairCode}"
        call ui.setValue "createToken" ""
        call ui.setValue "jobId" ""
      endif
    endif
  endif
  if $input.jobId != ""
    ...
    if $j != nil
      ...
      if $j.status == "completed"
        ...
        if $j.action == "delete_warehouse"
          call ui.setText "deleteStatus" "WAREHOUSE DELETED"
          call ui.alert "Warehouse deleted. Physical chest contents were not touched."
          call ui.navigate "spn://warehouse/my"
        endif
        call ui.setValue "jobId" ""
      endif
...
term.clear();term.setCursorPos(1,1);term.setTextColor(C.lime);print('WAREHOUSEOS PUBLISHER 1.3.0');term.setTextColor(C.white)
...
write('Publishing... ');call('web','publish',{domain=DOMAIN,note='WarehouseOS 1.3.0 reliable controller services + warehouse lifecycle'});print('OK')

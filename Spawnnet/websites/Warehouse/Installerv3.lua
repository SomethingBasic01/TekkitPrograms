...
local function immediateJob(j)
  ...
  elseif action=='create'then
    -- 1.3.0 no longer depends on the SpawnNet job completion round-trip to return
    -- the pair code. The website gives every create request a token. The Host
    -- publishes the exact result to a token-specific telemetry stream, which the
    -- create page reads directly. The job is still completed for normal auditing.
...
function M.ensureSupervisor()
  ...
  -- Deliberately do NOT use multishell. This call becomes a resident supervisor
  -- which runs the controller/terminal service and a normal CraftOS shell in parallel.
  -- From the player's point of view the command returns to a shell prompt, but the
  -- WarehouseOS worker remains alive underneath it.
  shell.run('/warehouseos/supervisor.lua','handoff')
  return true,'WarehouseOS supervisor stopped'
end
function M.startBackground(path,lock,...)
  -- Compatibility shim for old callers. 1.3.0 uses the supervisor instead of tabs.
...
local t=os.startTimer(1);local last=0;draw();while true do local ev={os.pullEventRaw()};if ev[1]=='rednet_message'and ev[2]==cfg.host and ev[4]==protocol then local obj,done=wire.accept(ev[2],ev[3]);if done and obj then handle(obj);draw()end elseif ev[1]=='timer'and ev[2]==t then local now=os.clock();common.writeRunning('/warehouseos/terminal.running');for id,r in pairs(pendingResults)do if now-r.created>12 then pendingResults[id]=nil elseif now-r.last>=1 then r.last=now;wire.send(cfg.host,r.msg,protocol)end end;if now-last>4 then last=now;heartbeat()end;draw();t=os.startTimer(1)elseif ev[1]=='key'and(ev[2]==keys.q or ev[2]==keys.escape)then break end end;gui.clear()
  ...
  ["/warehouseos/supervisor.lua"]=[=[-- WarehouseOS 1.3.0 resident supervisor.
  ...
  while true do
    if fs.exists(cfgPath)then
      if not common.isRunning(lock)then
        local ok,err=pcall(function() shell.run(program,'service') end)
...
local function shellLoop()
  while true do
    ...
    local ok,err=pcall(function()
      local resolved=(shell.resolveProgram and shell.resolveProgram('shell'))or'rom/programs/shell.lua'
      shell.run(resolved or'rom/programs/shell.lua')
    end)
...
if not common.supervisorRunning()then shell.run('/warehouseos/supervisor.lua','startup')end
]=],
}
local function mkdirFor(path) local d=fs.getDir(path);if d~=''and not fs.exists(d)then fs.makeDir(d)end end
term.clear();term.setCursorPos(1,1);term.setTextColor(colors.lime);print('WAREHOUSEOS 1.3.0');term.setTextColor(colors.white);print('Installing ME-style storage tools for SpawnNet...');print()
...
print('WarehouseOS no longer uses or repairs multishell at all.')
print('Controller/terminal services run beside a normal CraftOS shell through a resident parallel supervisor.')

...
local function slotScanMeta(slot)
  local geo, geoEntry = geolyzerProxyByAlias(slot.geoAlias)
  if not geo then return nil, nil, "Missing geolyzer " .. tostring(slot.geoAlias) end
...
local function captureSlot(slot)
  local meta, originalSlot, err = slotScanMeta(slot)
  ...
  for hx = 1, meta.sizeX do
    ...
    for hz = 1, meta.sizeZ do
      ...
      local column = meta.geo.scan(rx, rz, false)
      for relY = meta.yMin, meta.yMax do
        local idx = relY + 33
        local value = column and column[idx] or nil
        local v = classify(meta.mode, meta.airMax, meta.denseMin, value)
        if v ~= 0 then
          voxels[#voxels + 1] = {x = hx, y = relY - meta.yMin + 1, z = hz, v = v}
        end
      end
    end
  end

  originalSlot.lastUpdated = computer.uptime()
  originalSlot.lastCount = #voxels

  return {
    slotName = meta.slotName,
    geoAlias = meta.geoAlias,
    sizeX = meta.sizeX,
    sizeY = meta.sizeY,
    sizeZ = meta.sizeZ,
    mode = meta.mode,
    voxels = voxels,
    updatedAt = originalSlot.lastUpdated,
  }
end
  ...
  for i = 1, #slots do
    ...
    if not meta then
      ...
      for hx = 1, meta.sizeX do
        ...
        if (computer.uptime() - lastPing) > 1.5 then
          modem.send(toAddr, PORT, serialization.serialize({
            t = "geo.slot.progress",
            requestId = requestId,
            slotName = meta.slotName,
            columnsDone = hx,
            columnsTotal = meta.sizeX,
            count = sentCount,
          }))
          lastPing = computer.uptime()
        end
      ...
      originalSlot.lastUpdated = computer.uptime()
      originalSlot.lastCount = sentCount

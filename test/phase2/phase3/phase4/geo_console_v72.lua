...
local function requestSnapshots(node, slotNames)
  ...
  local expectedSlots = wanted and #wanted or nil
  local endedSlots = {}
  local endedCount = 0
  local sawAnyReply = false
  local slotTransfers = {}

  local function slotMetaFromNode(slotName)
    for i = 1, #((node and node.slots) or {}) do
  ...
  local function ensureTransfer(slotName)
    local t = slotTransfers[slotName]
    if not t then
      t = {meta = slotMetaFromNode(slotName), voxels = {}}
      slotTransfers[slotName] = t
    end
    return t
  end
  ...
  while not done and (computer.uptime() - lastPacket) < SNAPSHOT_IDLE_TIMEOUT do
    local fromAddr, packet = waitPacket(2)
    if packet then
      if fromAddr ~= node.addr then
        refreshNodeFromPacket(fromAddr, packet)
      elseif packet.requestId == requestId or packet.t == "geo.announce" then
        sawAnyReply = true
        lastPacket = computer.uptime()

        if packet.t == "geo.announce" then
          refreshNodeFromPacket(fromAddr, packet)
          node = db.geoNodes[node.nodeId] or node
        elseif packet.t == "geo.snapshot.start" and packet.requestId == requestId then
          if type(packet.totalSlots) == "number" and packet.totalSlots >= 0 then
            expectedSlots = packet.totalSlots
          end
        ...
        elseif packet.t == "geo.slot.chunk" and packet.requestId == requestId then
          local transfer = ensureTransfer(packet.slotName)
          if type(packet.voxels) == "table" then
        ...
        elseif packet.t == "geo.slot.end" and packet.requestId == requestId then
          ...
          if not endedSlots[packet.slotName] then
            endedSlots[packet.slotName] = true
            endedCount = endedCount + 1
          end
          if expectedSlots and endedCount >= expectedSlots then
            done = true
          end
  ...
  if sawAnyReply and endedCount > 0 then
    return true
  end
